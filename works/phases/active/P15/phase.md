...
- (F1) **The P15 MCP deploy requires `MCP_ALLOWED_HOSTS` (+ `MCP_ALLOWED_ORIGINS`)** — FastMCP's localhost-only DNS-rebinding default returns `421 Invalid Host header` to the public edge host `knowledge.hi2vi.com` and the internal `knowledge-mcp:9000` path otherwise. Protection stays ON (env-driven `config.allowed_hosts()`/`allowed_origins()` = localhost defaults + these vars, passed as an explicit `TransportSecuritySettings`). MATCHING: the port-less public host needs an **exact** allowlist entry (`knowledge.hi2vi.com`); the internal path uses `knowledge-mcp:*`. `compose.prod.yml` sets `MCP_ALLOWED_HOSTS="knowledge.hi2vi.com,knowledge-mcp,knowledge-mcp:*"`, `MCP_ALLOWED_ORIGINS="https://knowledge.hi2vi.com"` — operations doc. — **CONSOLIDATED at the re-review into operations `v0017`.**

### RE-REVIEW — consolidated P15.F1 → PASS (2026-07-18)

- **Verdict: `pass`.** Phase reopened only for P15.F1 (the `421` host-allowlist fix), already verified in prod by the orchestrator (public `GET /mcp` → routed `406`; authenticated `e2e_smoke.py` against `https://knowledge.hi2vi.com/mcp` with the master token → PASS). Re-review re-validated + consolidated the doc delta; **no code touched**.
- **Sanity re-validate:** `workflow.py validate` clean; `cd mcp-server && uv run pytest -q` → **11 passed** (prior 10 + F1 regression); `rebuild-docs` + `validate` clean post-rebuild.
- **Docs:** the `(F1)` Doc-impact line was operations-only → **operations `v0017`** (from v0016): new "Transport security: DNS-rebinding Host allowlist (P15.F1)" subsection (root cause, env-driven `TransportSecuritySettings`, the exact-vs-`:*` matching rule, the compose values), two env-var rows (`MCP_ALLOWED_HOSTS`/`MCP_ALLOWED_ORIGINS`), the "nothing live yet / operator post-deploy" phrasing flipped to **deployed + public-path verified**, and a new Invariant bullet. No api/architecture/product/security re-version.
- **Outstanding (operator, not phase gaps):** real hi2vi `vk_` provisioning + OpenClaw config (hi2vi P18.S5); **D13** `url` population via `source_url`.
