# Phase P8: Knowledge API for hi2vi content agent

_Intent: see [intent.md](intent.md)._

## Objective

Make the knowledge document API production-consumable by the hi2vi content agent: a reachable hosted endpoint (hosting proposed at DECOMP) with bearer auth, docs/hi2vi/ project-folder bootstrap, publish-on-write so agent-written docs (md file + DB row, /explain-style) land on main and GitHub Pages without operator action, and the existing read/search endpoints (hybrid search, get) exposed under the same auth so hi2vi can use accumulated knowledge for content creation

## Context

The write path (`POST /api/documents`) and the read/search surface already exist and are stable (P2/P4/P7). What is missing is **hosting**: today the API runs only on the operator's Mac (`compose.yml`, `localhost:8766`, bind-mount of the working tree, no push, no auth on reads, no TLS). This phase hosts it publicly at `https://knowledge.hi2vi.com` as a co-tenant on the shared OCI box, adds server-side push so writes publish to Pages automatically, gates reads/search behind the same bearer on the hosted deployment (local stays open), and freezes the API contract the hi2vi `P15.S4` content-agent client codes against.

The hosting shape (public subdomain on the shared edge) was **fixed by the operator at execution kickoff** (see intent.md addendum) — this DECOMP designs *within* that decision, it does not re-open it.

---

## Hosting Design Proposal — FOR OPERATOR SIGN-OFF

> **Sign-off gate (mandated by intent.md):** the operator approves this proposal before any implementation slice (P8.S1+) runs. Each numbered recommendation below is a decision the operator is signing off. Deviations become slice-plan notes.

**Fixed frame (not re-opened):** the knowledge API deploys **public at `https://knowledge.hi2vi.com`** — a new subdomain vhost on the shared OCI box's existing edge (`changple5-nginx-1` + Cloudflare + `changple_shared_network`), alongside hi2vi.com. One subdomain suffices; hi2vi's content agent consumes the public URL with a bearer token (no separate private-network path). GitHub Pages stays at `leetusik.github.io/knowledge/`.

### 1. Deployment shape on the OCI box — *recommend: own compose project + own clone, api-only, on the shared network*

- **Its own compose project** in this repo: `compose.prod.yml` shipping **only the `api` service** (no `kb` viewer — the public site is GitHub Pages; the box needs only the API). Smaller surface, no port 8765 on the box.
- **Its own git clone** on the box (e.g. `/opt/knowledge`), separate from the operator's Mac working tree. The container commits *and pushes* into this clone, so it must own a real clone with an `origin` remote (SSH) + push credential — **not** a working-tree bind mount. The box clone bind-mounts into the container at `/repo` as today.
- **Joined to `changple_shared_network`** (the existing external Docker network). The api container is reached by the edge over that network by container name (e.g. `knowledge-api:8000`); **no public host-port publish** for the api (nothing on 8766 exposed on the box's public interface).
- **Env on the box** (via a gitignored `.env`, secrets never in the repo): `KB_API_TOKEN=<generated>`, `KB_REQUIRE_READ_AUTH=true` (new, §3), `KB_GIT_PUSH=true` (new, §2), `KB_PUBLIC_BASE_URL=https://leetusik.github.io/knowledge` (so the 201 `url` points at the real published doc), `GOOGLE_API_KEY=<optional, §7>`, `TZ=Asia/Seoul`, `KB_STARTUP_REINDEX=true` (drift + fetch reconciliation on boot).
- **Edge (nginx vhost + TLS).** A `knowledge.hi2vi.com` server block on `changple5-nginx-1` that `proxy_pass`es to the api container over the shared network, using Docker internal DNS **with re-resolution** (`resolver 127.0.0.11 valid=30s` + a variable in `proxy_pass`, per the explainer's Option-B rule) so the proxy survives api-container recreation. TLS consistent with hi2vi.com: a **Cloudflare Origin CA cert covering `*.hi2vi.com`** (one cert already covers both hi2vi.com and knowledge.hi2vi.com if hi2vi uses a wildcard origin cert), referenced by the vhost. Cloudflare DNS: a proxied `knowledge` record for hi2vi.com (operator action, §5).
- **Edge fragility (documented, per `docs/hi2vi_web/2026-07-02-shared-nginx-explained.md` + deferred D2).** knowledge.hi2vi.com's conf + cert + network membership are **undeclared runtime state** on `changple5-nginx-1`, wiped on **every changple5 deploy** (the `depends_on` recreation cascade) — exactly like hi2vi.com today. What knowledge.hi2vi.com needs from the edge: (a) the vhost conf, (b) the `*.hi2vi.com` cert, (c) the edge attached to `changple_shared_network` (already true for hi2vi). **On an edge reset these must be re-applied.** The break-glass re-apply script (`deploy/edge/apply-to-edge.sh`) lives in the **hi2vi/changple5 repos, not here** — so P8 produces the knowledge vhost conf as a ready-to-drop artifact and a documented **cross-repo handoff**: "extend apply-to-edge.sh to also restore knowledge's conf + cert." Operational rule (same as hi2vi.com): **after any changple5 deploy, assume knowledge.hi2vi.com is down until the (extended) re-apply runs.** A documented re-apply step is acceptable; silent breakage is not. Long-term the Option-B dedicated edge (D2) folds knowledge in as a clean **fourth** conf drop-in.

### 2. Publish-on-write — *recommend: the hosted API pushes after its scoped commit; best-effort, 201 with `pushed`/`push_error`*

- **Mechanism:** after the existing scoped `git commit`, the write path pushes to `origin/main`. Gated by a **new `KB_GIT_PUSH` flag (default false)** so local/plugin deployments **never** push (preserves the "agent never pushes locally" convention); the box sets it true. This is the deliberate, scoped departure from that convention that intent.md point 4 authorizes.
- **Credential:** a **repo-scoped SSH deploy key** with write access (better blast radius than a PAT that spans all the operator's repos; no expiry to babysit). Public half added as a GitHub **deploy key (allow write)** on `leetusik/knowledge`; private half placed on the box and mounted read-only into the container (`GIT_SSH_COMMAND` / mounted `~/.ssh`). The box clone's `origin` uses the SSH URL. (Fine-grained PAT with `contents:write` is the fallback if SSH is inconvenient.)
- **Push discipline — fetch + rebase, never force.** Before pushing: `git fetch origin main`, then rebase the api's commit(s) onto `origin/main`, then `git push origin HEAD:main`. The api only ever **adds a new date-slug doc file** (unique path) and **appends** a Recent bullet to `docs/index.md`, so a rebase onto operator commits is virtually always clean; a genuine conflict (operator edited the same `docs/index.md` region) → abort the rebase, keep the local commit, surface `push_error`. **Never `--force`.**
- **Failure semantics — mirror `committed`/`commit_error` exactly.** Push is a best-effort publish step, like the commit already is. Success/failure never changes the write outcome: still **201**, now carrying `pushed: true|false` and, on failure, `push_error`. A failed push means the doc is written + committed on the box but not yet on Pages — it publishes on the **next** successful push (commits accumulate) or a manual push. **No 5xx** for a push failure. Applies to every commit-producing mutating endpoint (POST documents; DELETE too, for consistency).
- **Divergence (operator also pushes to main from the Mac):** handled by the fetch+rebase-before-push above — the box always lands its commit on top of the latest remote, so it never reverts the operator's work. A failed push never loses the box's commit (local, retried next write).

### 3. Auth on reads (hosted) — *recommend: new `KB_REQUIRE_READ_AUTH` flag; healthz stays open; no CORS*

- **New `KB_REQUIRE_READ_AUTH` flag (default false).** When false (local/plugin default) reads/search stay **open** — the open-by-default local dev + plugin UX intent.md protects. When true **and** `KB_API_TOKEN` is set (the box), the existing `require_bearer` dependency is applied to the read/search endpoints (`GET /api/documents`, `/api/documents/{id}`, `/api/documents/by-path`, `/api/search`, `/api/tags`, `/api/projects`) → 401 without the bearer. Writes are already bearer-guarded regardless.
- **`healthz` stays open** even under read-auth (edge/uptime/Cloudflare probes want it). Its `documents` count leak is **immaterial**: the entire corpus is already public on Pages, so the count is derivable public information (intent.md: "a public API surface leaks nothing new"). Reads are gated not for content secrecy but to stop unauthenticated abuse/load on the box's api. No healthz change needed.
- **CORS: none.** The consumer is **server-to-server** (the hi2vi agent runs server-side); the public Pages site searches **browser-only via lunr and never calls this API** (per architecture.md's search-boundary). So no browser origin ever calls the hosted api → no CORS config. State it explicitly; a future browser client would be a separate change.

### 4. Hosted clone freshness — *recommend: fetch+rebase-before-push (§2) is the freshness mechanism; + boot fetch/reindex*

- The **push flow itself keeps the clone fresh**: every write fetches `origin/main` and rebases onto it before pushing, so the box catches up on operator commits at each write. No cron, no webhook — a daily-write agent needs no real-time mirror.
- **A stale clone must not (a) cause push rejections** — the rebase-before-push handles it; **or (b) resurrect deleted docs** — the write path stages only its own touched paths (`git add --`, never `-A`) and rebases onto the latest remote, so the box's push is always on top of the operator's deletes, never a revert.
- **Boot self-heal:** `KB_STARTUP_REINDEX=true` on the box (fetch on boot, then reindex) reconciles the DB projection with any operator deletes/edits pulled in. Deleted-doc reconciliation of the box's search index is eventually-consistent (next boot / next full reindex) — acceptable for a single daily writer. A periodic pull+reindex is a possible later add, not needed at launch.

### 5. Secrets provisioning + frozen consumer contract — *recommend: operator places secrets on the box (pending handoff); freeze the contract into api.md at review*

- **Secrets on the box (never in this repo), all operator actions (pending handoffs):**
  1. `KB_API_TOKEN` — generate a strong random token; place in the box's gitignored `.env`.
  2. **Git push credential** — generate an SSH keypair; add the public key as a GitHub **deploy key (write)** on `leetusik/knowledge`; place the private key on the box.
  3. **Cloudflare** — add a proxied `knowledge` DNS record for hi2vi.com; ensure the `*.hi2vi.com` origin cert covers it.
  4. Optional `GOOGLE_API_KEY` (§7).
- **Frozen consumer contract** (what hi2vi `P15.S4` codes against — freeze it so the cross-repo client is stable):
  - Config: `KNOWLEDGE_API_URL=https://knowledge.hi2vi.com`, `KNOWLEDGE_API_TOKEN=<the KB_API_TOKEN>`.
  - Write `POST /api/documents` → **201** `{id, rel_path, url, title, project, slug, date, tags, related, recent_updated, landing_created, committed, commit_sha, pushed, push_error?}` (the `pushed`/`push_error` fields are the P8 additions; existing consumers ignore them) / **409** duplicate `{message, rel_path, id, existing_title}` / **422** convention / **401** auth.
  - Read/search: `GET /api/search?q=…`, `GET /api/documents`, `GET /api/documents/{id}` etc. — response shapes exactly as `docs/current/api.md` — now **requiring `Authorization: Bearer <token>`** on the hosted deployment.
  - **Where it lives:** consolidated into `docs/current/api.md` at P8.REVIEW (a "Hosted deployment + frozen contract" section), published on Pages — that published api.md **is** the pointer the hi2vi repo references. (Doc-impact recorded below.)

### 6. `docs/hi2vi/` bootstrap — *recommend: verify-only (auto-landing already covers it); fold into the E2E check*

- No pre-creation needed. The first `POST /api/documents` with `project: "hi2vi"` triggers P7.F1 `ensure_project_landing` → auto-creates `docs/hi2vi/index.md` (staged in the same scoped commit), satisfying the per-project `site/hi2vi/index.html` deploy-gate invariant. So the first hi2vi write cannot break `site_smoke.py`.
- **Naming is distinct and confirmed:** research docs go to `docs/hi2vi/` (`project: "hi2vi"`); the existing engineering explainers stay in `docs/hi2vi_web/` (`project: "hi2vi_web"`). Keep them separate.
- Work reduces to an **E2E "first hi2vi write" verification** (does the auto-landing + deploy gate hold end-to-end) — folded into the acceptance slice, not a separate build.

### 7. Embeddings on the box — *recommend: provision the Gemini key at launch*

- Provision `GOOGLE_API_KEY` on the box. hi2vi's read/search use case (topic dedup, research grounding — intent.md point 5) is exactly what hybrid semantic search improves; it is one env var with graceful BM25-only degradation if absent or quota-limited, and the low quota is fine for one daily agent. If the operator prefers, launch BM25-only and add the key later with **zero code change** — but the recommendation is to provision it.

---

## Decomposition

Five middle slices (`P8.DECOMP` + `P8.REVIEW` already existed). Risk drives the executor tier and is the main cost lever — nothing touching gitops/push, auth, or the prod deploy is `low`.

| Slice | Name | Kind | Risk | Order | Depends |
|---|---|---|---|---|---|
| **P8.S1** | publish-on-write: server-side git push after the scoped commit | implementation | **high** | 1 | — |
| **P8.S2** | hosted read auth: gate reads/search behind bearer (local stays open) | implementation | medium | 2 | S1 |
| **P8.S3** | prod deploy artifacts for knowledge.hi2vi.com (compose.prod + vhost + runbook) | implementation | medium | 3 | S2 |
| **P8.S4** | secrets provisioning runbook + frozen consumer contract | implementation | medium | 4 | S3 |
| **P8.S5** | E2E acceptance: first hi2vi write → push → Pages → live; search under auth | implementation | medium | 5 | S4 |

**Rationale + scope per slice** (the executor writes each `plan.md` at its own turn — these are scope sketches, not plans):

- **P8.S1 — publish-on-write (risk: high).** The biggest, most delicate item and the phase's core new capability: server-side `git push` (§2). Add a `push()` to `server/gitops.py` (fetch + rebase-onto-`origin/main` + non-force push), a `KB_GIT_PUSH` config flag (default false → local never pushes), wire it into the commit-producing mutating paths, and add `pushed`/`push_error` to the 201/DELETE bodies with best-effort semantics mirroring `committed`/`commit_error`. Tests against a **local bare remote** (no network/credentials needed in the executor's env). **High** because it touches gitops/push and a bad push discipline could clobber `main` — it needs full judgment on rebase/divergence/failure semantics.

- **P8.S2 — hosted read auth (risk: medium).** Add the `KB_REQUIRE_READ_AUTH` flag (§3); apply the existing `require_bearer` to the read/search routes only when it is true + `KB_API_TOKEN` set; keep healthz open; no CORS. Small, well-scoped, but touches auth → not `low`. Tests: reads open by default, 401 under the flag without bearer, 200 with it.

- **P8.S3 — prod deploy artifacts (risk: medium).** Author the box artifacts (§1): `compose.prod.yml` (api-only, own-clone mount, external `changple_shared_network`, the box env incl. the new flags), the `knowledge.hi2vi.com` nginx vhost conf (re-resolution + TLS), and a **runbook** (box clone setup, bring-up, the edge re-apply + cross-repo `apply-to-edge.sh` handoff note). **No secrets in any artifact** (env by name only). Operator SSH-applies later; this slice produces ready-to-run artifacts + a validate-after-apply gate. Prod deploy → not `low`.

- **P8.S4 — secrets provisioning + frozen contract (risk: medium).** Two things: (a) a **secrets/DNS provisioning runbook** shaped as explicit **operator pending handoffs** (KB_API_TOKEN, deploy key + GitHub deploy-key add, Cloudflare DNS record, optional Gemini key — §5); (b) **freeze the consumer contract** hi2vi `P15.S4` codes against (§5) and record the doc-impact so the review consolidates it into `api.md`. Defines the cross-repo contract → judgment, medium.

- **P8.S5 — E2E acceptance (risk: medium, operator-gated).** After the box is live: verify `docs/hi2vi/` bootstrap (auto-landing, distinct from `docs/hi2vi_web/`), then the full acceptance — a `project:"hi2vi"` write to `https://knowledge.hi2vi.com` → 201 `pushed:true` → commit on `main` → Pages deploy passes `site_smoke.py` → doc live at the Pages URL; `GET /api/search` + get read back **under bearer**; **401** without the token. Requires the live endpoint → an operator-applied gate precedes it (pending). Validation-heavy + cross-repo → medium.

**Pending handoff points** (operator-only actions, flagged so the orchestrator sees them coming):
- P8.S3: box clone setup + `docker compose -f compose.prod.yml up`, edge conf/cert drop + `apply-to-edge.sh` extension (cross-repo), on the OCI box (assume the agent has **no SSH access** — these are operator-applies-then-validates gates).
- P8.S4: KB_API_TOKEN generation, SSH deploy-key generation + GitHub deploy-key registration, Cloudflare DNS record, optional Gemini key.
- P8.S5: the endpoint must be live (operator brought it up) before the E2E runs; likely a `pending` gate on the operator to confirm bring-up.

**Cross-repo ordering (operator asked "which order, for both repo"):**
- hi2vi `P15.S1–S3` do **not** depend on P8 — run independently, any time.
- hi2vi `P15.S4` (research + knowledge-write client) consumes P8's **frozen contract** at planning time → freeze it (P8.S4, which depends on the S1/S2 shapes) before P15.S4 plans. P8.S4's contract-freeze is the cross-repo unblock and can be prioritized once S1/S2 land.
- hi2vi `P15.S9` (e2e) needs the **live endpoint** → P8.S3+S4+S5 (deploy + secrets + acceptance) must complete before P15.S9.
- P7 (this repo, plugin) is independent of both.

## Findings & Notes

Verified ground truth (recon spot-checks this DECOMP ran — trust the plan's recon, these confirm what the slices build on):

- **Auth is a plain FastAPI dependency.** `require_bearer` (`server/main.py:64`) is a no-op when `KB_API_TOKEN` is unset, else exact-matches `Authorization: Bearer <token>` → 401. Reads (`/api/documents`, `/api/search`, `/api/tags`, `/api/projects`, `healthz`) currently take **no** auth dependency. Gating reads (S2) = adding a flag-conditional dependency to those routes — clean, small.
- **`server/gitops.py` has no push/remote/credential code at all** — `add()` (scoped `--`, never `-A`) + `commit()` (returns HEAD sha) only, every failure → `GitError`. S1 adds `push()` net-new; there is nothing to unwind.
- **Config is env-at-call-time** (`server/config.py`, never cached at import) — adding `KB_GIT_PUSH` / `KB_REQUIRE_READ_AUTH` follows the existing `git_commit_enabled()` / `api_token()` pattern exactly (falsy parse `{0,false,no,off}`).
- **The image already ships `git`** and system-level identity + `safe.directory` (`Dockerfile`), so `push()` in-container needs only the SSH credential + remote — no image changes for the push mechanic itself (a deploy-key mount is a compose concern).
- **`compose.yml` today is local-only**: bind-mounts the working tree (`.:/repo`), publishes 8765/8766, no auth/push/TLS/prod env. The prod compose (S3) is a **separate file**, api-only, own-clone — it does not touch the local `compose.yml`.
- **`KB_PUBLIC_BASE_URL` is the viewer origin for the 201 `url` field** (`config.py:39`), *not* the API's own origin — set it to the Pages URL on the box (`site_url` is `https://leetusik.github.io/knowledge/`) so `url` points at the real published doc. Distinct from `KNOWLEDGE_API_URL` (the API origin the consumer hits).
- **`docs/hi2vi/` does not exist yet** (only `docs/hi2vi_web/`), so the first `project:"hi2vi"` write auto-creates its landing via P7.F1 — bootstrap is verify-only.
- **Origin remote is `https://github.com/leetusik/knowledge.git`** — the box clone will use the SSH form (`git@github.com:leetusik/knowledge.git`) for deploy-key push.
- **Rebuild quirk (operations.md):** `COMPOSE_BAKE=false docker compose up -d --build` on this host — carry the same note into the prod bring-up runbook.

### P8.S1 — publish-on-write (delivered)

- **`gitops.push()` is net-new** (`server/gitops.py`): `git fetch origin main` → `git rebase origin/main` → `git push origin HEAD:main`; returns the **final pushed HEAD** sha (a rebase may rewrite the commit). On any step failing it runs a best-effort `git rebase --abort` (raw `subprocess.run`, its own non-zero swallowed — harmless when nothing to abort) then re-raises `GitError`. Never `--force`, never `add -A`. Signature `push(*, root, remote="origin", branch="main")` — callers just pass `root`; the params exist for tests.
- **Verified empirically** (bare-remote scratch, mirrored in `tests/test_api_push.py`): `git fetch origin main` opportunistically updates `refs/remotes/origin/main` (the standard clone refspec), so `rebase origin/main` targets the freshly-fetched tip; divergence → our commit lands on top with the operator commit still an ancestor (no clobber); a conflicting operator edit to the same `docs/index.md` Recent region → rebase conflicts, abort restores the local commit, no `.git/rebase-merge|rebase-apply` left, tree clean.
- **Config flag `KB_GIT_PUSH` defaults FALSE** (`server/config.py:git_push_enabled`) — note the **inverted default** vs `git_commit_enabled`: truthy-parse `{1,true,yes,on}`, so local/plugin never pushes. Only the box sets it true.
- **Wiring** (`server/main.py`): push runs **inside `WRITE_LOCK`**, immediately after the commit, only when `committed and git_push_enabled()`, in both `create_document` (POST) and `_delete_document` (both DELETE handlers). On success `commit_sha` is reassigned to the pushed HEAD; on failure/disabled it keeps the local `commit()` sha.
- **Response contract:** `pushed: bool` always present (POST 201 + DELETE 200); `push_error` present only when a push was attempted and failed. Push failure is **never** a 5xx — mirrors `committed`/`commit_error` exactly.
- **Gotcha for later slices:** the test fixture bare remote needs `git init --bare -b main` (else its HEAD defaults to `master` and clones don't check out `main`, so `push origin main` fails with "src refspec main does not match any"). Relevant to any S3/S5 artifact or E2E that clones the box repo. Also, keep `KB_DB_PATH` outside the work tree in push tests so the disposable `data/` never dirties the clean-tree assertion.
- **DELETE also pushes** (settles the DECOMP open question "DELETE push scope") — parity with POST for the reverse path.

### P8.S2 — hosted read auth (delivered)

- **New config accessor `require_read_auth_enabled()`** (`server/config.py`) copies the `git_push_enabled()` idiom exactly: env-at-call-time, truthy-parse `{1,true,yes,on}`, **default false** (the inverted default — like `git_push_enabled`, unlike the falsy-parsed `git_commit_enabled`/`startup_reindex_enabled`). So local/plugin reads stay open unless the box explicitly sets `KB_REQUIRE_READ_AUTH=true`.
- **New dependency `require_read_bearer`** (`server/main.py`, right after `require_bearer`) **delegates to `require_bearer`** rather than reimplementing the check: `if not require_read_auth_enabled(): return` else `require_bearer(authorization)`. This encodes the both-must-hold rule for free — flag off → open; flag on + no token → `require_bearer` no-ops → open; flag on + token set → identical exact-match 401. Reuses the write-path check byte-for-byte, so the two auth surfaces can never drift.
- **Attached to the six read/search routes only** (`GET /api/documents`, `/api/documents/{doc_id}`, `/api/documents/by-path/{rel_path}`, `/api/search`, `/api/tags`, `/api/projects`) as `_: None = Depends(require_read_bearer)`, inserted after each route's path/required params and before `conn` so default-arg ordering stays valid. `require_bearer` and every write route are untouched.
- **`GET /healthz` deliberately left ungated** — always open for edge/uptime/Cloudflare probes; the `documents` count leak is immaterial (the corpus is public on Pages). No healthz change.
- **No CORS — a deliberate, recorded decision.** No CORS middleware was added: the consumer is server-to-server (the hi2vi agent runs server-side) and the public Pages site searches browser-only via lunr and never calls this API, so no browser origin ever hits the hosted API. A future browser client would be a separate change. (Recording this here per the plan so the decision is on the record as intentional, not an omission.)
- **Backward-compat guard is the key invariant** and is tested: `KB_API_TOKEN` set + flag unset → reads still 200 with no header (a set token guards **writes only** locally, unchanged). Verified alongside flag-on 401/200 and the flag-on-but-tokenless no-op.
- **Module docstring updated** (`server/main.py`) — it had gone stale ("Reads … are always open"); now documents the flag-gated read auth, the always-open healthz, and the no-CORS decision. `docs/current/*.md` untouched (durable truth flows through Doc impact → P8.REVIEW).
- **Test/tooling note:** pytest lives only in the repo `.venv` (`.venv/bin/python -m pytest`); the Homebrew `python3` (3.13) has no pytest. Full suite is now **65 passed** (was 62 at S1; +3 read-auth tests). Relevant to S5's E2E: exercising 401-without-token / 200-with-token against the live box mirrors these unit assertions.

### P8.F1 — plugin template sync for S1/S2 changes (delivered)

- **Parity restored.** `python3 scripts/plugin_parity.py` was FAIL with exactly the 5 issues P8.S3 flagged: byte drift in `server/config.py`, `server/gitops.py`, `server/main.py`, `tests/test_api_read.py` (all changed by S1/S2 without mirroring into `plugin/templates/kb/`), and `tests/test_api_push.py` (new in S1, not shipped at all). Fixed by copying all five files byte-identically into `plugin/templates/kb/` and adding `"tests/test_api_push.py"` to `manifest.json`'s `files.identical` array (before `tests/test_api_read.py`, alphabetical). `plugin_parity.py` now PASSes with 0 issues.
- **Plugin version bumped 0.1.0 → 0.2.0** (`plugin/.claude-plugin/plugin.json`) per the release-checklist rule (new backward-compatible server capability + response fields ⇒ minor bump). Grepped for other occurrences of the plugin's version string: found `"plugin_version": "0.1.0"` in `plugin/skills/setup/SKILL.md` (the scaffold marker JSON written into new KB projects) — clearly the plugin's own version, bumped identically to 0.2.0. Also found `version = "0.1.0"` in `plugin/templates/kb/pyproject.toml`, `plugin/templates/kb/uv.lock` (the `kb-api` package entry), and `FastAPI(title="kb-api", version="0.1.0", ...)` in both root and template `server/main.py` — confirmed these are the **scaffolded kb-api application's own version** (a separate product from the Claude plugin), unchanged from root (root's `server/main.py` also carries `version="0.1.0"` for the FastAPI app, untouched by S1/S2) — not the plugin's version, left as-is, not ambiguous.
- **Process lesson:** a slice that edits `server/*`/`tests/*` payload files must run `scripts/plugin_parity.py` locally as part of its own validation — pytest alone doesn't catch template drift because parity only runs in CI (`plugin-ci.yml`) on push. Add this check alongside pytest whenever a slice's plan touches shipped-payload paths.
- **Validation:** `python3 scripts/plugin_parity.py` → PASS, 0 issues. `.venv/bin/python -m pytest -q` → 65 passed (unchanged). `python3 scripts/workflow.py validate` → passed.

### P8.S3 — prod deploy artifacts (delivered)

- **Artifacts (authoring + local validation only, no `pending`):** `compose.prod.yml` (repo root), `deploy/knowledge.hi2vi.com.conf`, `deploy/README.md` (runbook), + `.env` added to `.gitignore`. No server code, local `compose.yml` untouched. Operator SSH-applies before P8.S5.
- **`compose.prod.yml` shape:** only the `api` service; `build: .`; `container_name: knowledge-api`; **no published ports**; attaches `changple_shared_network` (`external: true`); box clone bind-mounts `.:/repo` (compose file lives in the box clone `/opt/knowledge`). Non-secret box config is literal in `environment:` (`KB_GIT_PUSH=true`, `KB_REQUIRE_READ_AUTH=true`, `KB_PUBLIC_BASE_URL=https://leetusik.github.io/knowledge`, `KB_ROOT=/repo`, `TZ=Asia/Seoul`, `KB_STARTUP_REINDEX=true`); **secrets by reference only** via `env_file: .env` (`KB_API_TOKEN`, `GOOGLE_API_KEY`) — no secret values in the repo. `restart: unless-stopped`.
- **Push credential wiring (decision):** deploy key + `known_hosts` bind-mounted read-only from `/opt/knowledge-secrets:/run/secrets:ro`; `GIT_SSH_COMMAND=ssh -i /run/secrets/knowledge_deploy_key -o IdentitiesOnly=yes -o UserKnownHostsFile=/run/secrets/known_hosts -o StrictHostKeyChecking=yes`. **Chose pinned `known_hosts` over `accept-new`** — github.com host keys are static + publicly verifiable, so pinning closes the first-push TOFU MITM window; `accept-new` documented as the weaker alternative. Settles the "push credential form → SSH deploy key" open question at the deploy layer (the `push()` code is URL-agnostic; S4 picks SSH-key vs PAT for provisioning).
- **vhost decision:** Docker-DNS re-resolution per the explainer (`resolver 127.0.0.11 valid=30s` + `set $kb_upstream http://knowledge-api:8000` + `proxy_pass $kb_upstream`) so the proxy survives api recreation; TLS references the `*.hi2vi.com` wildcard origin cert with a CONFIRM-AT-APPLY comment + a commented per-host-cert variant; `client_max_body_size 5m`; port 80→443 redirect. HTTP/2 left as a version-matched comment to avoid an `nginx -t` failure across nginx versions.
- **Enhancement (deviation):** added a dependency-free `healthcheck` to `compose.prod.yml` (image python → `/healthz`, `start_period: 60s` for the boot reindex) so the runbook's "container healthy" step is real. Low-risk; not in the plan's shape list.
- **Validation:** `docker compose -f compose.prod.yml config` → PASS (dummy gitignored `.env`, since removed); `nginx -t` on nginx:1.27-alpine with a stub self-signed cert (`--network none`) → PASS ("syntax is ok / test is successful", validating resolver + variable `proxy_pass` + TLS load); pytest **65 passed** (unchanged); `workflow.py validate` → PASS.
- **⚠️ Pre-existing plugin_parity FAIL — needs a decision before the next push.** `python3 scripts/plugin_parity.py` fails with 5 drift issues, **all from committed S1/S2 changes not mirrored into `plugin/templates/kb/`**: byte drift in `server/config.py`, `server/gitops.py`, `server/main.py`, `tests/test_api_read.py`, and `tests/test_api_push.py` present in repo but not shipped. P8.S3 touches none of these paths (its new files — `compose.prod.yml`, `deploy/*`, `.gitignore` — are not parity-tracked), so it neither caused nor can fix this. Parity runs only in CI (`plugin-ci.yml`) on push, so S1/S2's local pytest gate never caught it. **Action needed:** a mechanical plugin-template sync (copy the 3 changed `server/*.py` + `tests/test_api_read.py` into `plugin/templates/kb/`, add `tests/test_api_push.py` to the template + `manifest.json` `identical` list) — its own fix slice or folded into P8.REVIEW — else `plugin-ci` fails on the next push. **Sub-question for that sync:** the new flags default off (local/plugin never push, reads open), so shipping the changed server code to plugin users is behavior-safe; the sync is byte-parity housekeeping only.

### P8.S4 — secrets provisioning runbook + frozen consumer contract (delivered)

- **Two authoring-only deliverables, no server/test/docs edits, no `pending`** (the operator runs the runbook at the pre-S5 gate): `deploy/SECRETS.md` (new) + `works/phases/active/P8/contract.md` (new); `deploy/README.md` pointer-wiring only. pytest **65 passed** (unchanged — nothing behavioral), `plugin_parity.py` PASS (touched no shipped paths), `workflow.py validate` PASS.
- **`deploy/SECRETS.md` = the "produce + register" half; `README.md` = the "place + bring up" half** — split deliberately to avoid duplicating steps. SECRETS.md owns: (1) `KB_API_TOKEN` generation (`openssl rand -hex 32`) + the note that this same value is hi2vi's `KNOWLEDGE_API_TOKEN` (single shared cross-repo secret); (2) SSH deploy-key generation (`ssh-keygen -t ed25519 -N '' -C knowledge-api@oci`) + GitHub deploy-key registration with **write access**, private half → `/opt/knowledge-secrets/knowledge_deploy_key` (chmod 600, the exact `compose.prod.yml` mount path), origin must be the SSH URL; (3) Cloudflare proxied `knowledge` record + a **cert-coverage check** (openssl SAN read on the edge cert → wildcard `*.hi2vi.com` reuse vs. per-host cert issue + vhost variant); (4) optional `GOOGLE_API_KEY`; (5) a handoff checklist ending "proceed to README §1". Placement mechanics (known_hosts pinning, .env writing) are **referenced to README §1b/§1c**, not repeated. `README.md`'s four `P8.S4` slice-ID pointers are now explicit `SECRETS.md` links.
- **`contract.md` shapes are VERIFIED, not guessed** — exercised every endpoint via the pytest `TestClient` over a temp KB tree (scratch probe, repo untouched). Ground truth captured: **201** key order `id, rel_path, url, title, project, slug, date, tags, related, recent_updated, landing_created, committed, commit_sha, pushed` (+ optional `commit_error`/`push_error`); **409** = `{"detail": {message, rel_path, id?, existing_title?}}` (id/existing_title only when a DB row exists — bare on-disk collision omits them); **422** = `{"detail": "<reason string>"}`; **401** = `{"detail": "missing or invalid bearer token"}`. Search item keys: `id, project, slug, date, title, tags, rel_path, source_repo, created_at, updated_at, score, snippet, signals`; `signals={bm25?, recency, vector?}`. List item drops `markdown`; get-by-id/by-path adds it. `url` on the box = `https://leetusik.github.io/knowledge/<project>/<date>-<slug>/` (KB_PUBLIC_BASE_URL is the Pages origin). `pushed:false` on the probe confirms the field is always present even with push disabled.
- **`contract.md` is verbatim-ready for `api.md`** — everything below its horizontal rule is a drop-in `## Hosted deployment + frozen contract` section; P8.REVIEW consolidates it via `doc-new-version --doc api` (not hand-edited). Freeze rule stated: additive-only after P8 (new fields/endpoints may appear; existing field names/types/status-code meanings won't change; clients ignore unknown keys).
- **Settled the two S4 open questions at the provisioning layer:** push-credential form = **SSH deploy key** (recommended; `push()` is URL-agnostic so PAT stays a fallback), and Gemini-at-launch = **provision recommended, BM25-only launch supported with zero code change** — both documented as operator choices in SECRETS.md.
- **Retry/idempotency rule for the client (from code):** re-POSTing the same `project/date/slug` after an ambiguous failure → **409** (`rel_path` exists) → treat as already-written, read back via `GET /api/documents/by-path/{rel_path}`; never retry the write on `pushed:false` (the doc is written+committed, publishes on the next push).

### P8.F2 — reality-fix: `openssh-client` + deploy artifacts retargeted to the live edge (delivered)

Two artifacts-vs-reality defects, both caught by SSH reconnaissance on the box and both fixed. **The recon below is verified live — later slices and the review should trust it over anything S3/S4 assumed.**

- **DEFECT 1 (silent killer): the shipped image could not push over SSH.** `Dockerfile` installs git with `--no-install-recommends`, and `ssh` is only a *Recommends* of git — so the image had **no `ssh` binary** (control: `docker run --rm python:3.12-slim sh -c 'command -v ssh'` → nothing). `KB_GIT_PUSH=true` pushes to `git@github.com:…` via `GIT_SSH_COMMAND`, so **every push would have failed** — and because P8.S1 made push best-effort, it would have failed **SILENTLY**: every write still 201, with `pushed:false` + `push_error`, and nothing ever reaching Pages. The phase's core capability, dead on arrival, with no loud signal. Fixed by adding `openssh-client` to the apt line; **proven** by rebuilding the image → `/usr/bin/ssh`, OpenSSH_10.0p2. Mirrored byte-identically into `plugin/templates/kb/Dockerfile`; plugin version **0.2.0 → 0.2.1** (`plugin.json` + the `plugin_version` scaffold marker in `skills/setup/SKILL.md`); `plugin_parity.py` PASS.
  - **Lesson (generalizes):** a best-effort code path can hide a *total* infrastructure failure. When a capability is best-effort by design, its enablement must be asserted at bring-up — `README.md` §2 now asserts `ssh` is in the image. P8.S5's E2E must check `pushed:true`, never just the 201.
- **DEFECT 2: S3/S4's artifacts targeted an edge that no longer exists.** They were authored against `docs/hi2vi_web/2026-07-02-shared-nginx-explained.md`, which describes the **old shared edge**. That is superseded — the box now runs the dedicated "Option B" edge that explainer *proposed* (deferred **D2**'s end state, cut over 2026-07-02; the box carries a `.cutover-anchor`).

**The real edge topology (verified live — this is the ground truth):**

- Compose project **`edge`** at **`/home/opc/edge`**, container **`edge-nginx`** (`nginx:1.27-alpine`, up 11 days), sole owner of `0.0.0.0:80` + `:443`, on **`changple_shared_network`** only. **`changple5-nginx-1` DOES NOT EXIST.**
- **`conf.d/` + `certs/` are READ-ONLY HOST BIND MOUNTS** ⇒ the edge's config is **declarative host state**. A config change is a **host file drop + a reload**. The edge project has no `depends_on` and no `build`, so **a co-tenant (changple5) deploy can no longer wipe it** — the old "assume knowledge.hi2vi.com is DOWN after any changple5 deploy" rule and the cross-repo `apply-to-edge.sh` handoff are **obsolete and deleted**. There is no `apply-to-edge.sh` on the box and none is needed.
- Apply path is the edge's **own tooling**: **`./deploy.sh`** = hard `nginx -t` gate *inside the running container* → graceful `nginx -s reload`, and **never a recreate** (recreation drops the network attachment — the cascade the dedicated edge exists to remove). **`./validate.sh`** = local pre-gate. **Never `docker cp` into `edge-nginx`, never `docker compose up`/`restart` the edge.**
- Existing vhosts: `00-default.conf` (owns BOTH `default_server` catch-alls → `444`), `changple5.conf`, `changple-web.conf`, `hi2vi.conf`. Ours joins as `knowledge.conf`.
- **Cert: nothing to provision.** `/home/opc/edge/certs/hi2vi.crt` is a Cloudflare Origin CA cert with SANs **`DNS:*.hi2vi.com, DNS:hi2vi.com`**, valid to **2041** — the **wildcard already covers `knowledge.hi2vi.com`**. This settles the DECOMP open question "TLS cert mechanism": **wildcard, confirmed**. The per-host-cert branch is dead code and was deleted from the vhost.
- **DNS is live**: `knowledge.hi2vi.com` resolves to the same Cloudflare proxy IPs as `hi2vi.com`.

**Two edge hazards worth carrying forward (they bite the whole edge, not just us):**

- **The `conf.d/` tree is tested and reloaded AS A UNIT.** A `limit_req_zone` name is **global across the tree**; `hi2vi.conf` declares the only one (`hi2vi_contact`). A duplicate is a hard `nginx -t` failure that blocks the reload **for every site on the box**. Our vhost declares **none** (every `/api/*` call is bearer-gated, Cloudflare-fronted, one known consumer). **Empirically demonstrated:** appending a duplicate zone to our conf failed the whole-tree test (`[emerg] … already bound … knowledge.conf:141`).
- **No IPv6 `listen [::]:…`.** No sibling conf listens on v6, so adding it would silently make *our* vhost the **default server for all v6 traffic** on that port. House style omits it; so do we.

**Artifacts (this slice):** `deploy/knowledge.hi2vi.com.conf` → **`git mv` → `deploy/knowledge.conf`** (short-name convention), body rewritten on the `hi2vi.conf` house pattern: `:80 → 301`; `:443 ssl` + `http2 on`; wildcard cert; Docker-DNS re-resolution (`resolver 127.0.0.11 valid=30s ipv6=off` + `set $knowledge_upstream knowledge-api` → `proxy_pass http://$knowledge_upstream:8000`); CF real-IP restore; HSTS `max-age=300`; `client_max_body_size 5m`; `proxy_read_timeout 120s` (**Cloudflare itself 524s at ~100s** — the origin ceiling is not a promise). `deploy/README.md` + `deploy/SECRETS.md` rewritten for the real box. `compose.prod.yml` **header comment only** (it pointed at the dead edge + the now-deleted conf filename; no config line touched).

**Secrets discipline tightened (SECRETS.md):** no secret value ever transits a laptop/chat/transcript. The **deploy key is generated ON THE BOX** into `/opt/knowledge-secrets/knowledge_deploy_key` (private half never leaves); only the `.pub` moves, registered with `gh repo deploy-key add … --allow-write`. **`KB_API_TOKEN` is generated in place** into `/opt/knowledge/.env` (unquoted heredoc ⇒ `$(openssl rand -hex 32)` expands as the file is written), read back on demand with `ssh oracle-cloud "sudo grep ^KB_API_TOKEN= /opt/knowledge/.env"`.

**Bring-up facts for P8.S5 / the orchestrator:**

- **The clone must be `opc`-owned.** `docker compose` reads `compose.prod.yml` + `.env` **client-side, as the invoking user** (`opc`, in the `docker` group). A root-owned `.env` at mode 600 is unreadable ⇒ bring-up fails. README §1 now does `chown opc:opc /opt/knowledge`, `.env` 600 owned by `opc`. (The container itself runs as **root** and writes git objects into the clone as root — expected; host-side `git` there may need `sudo`.)
- **The repo is public** (unauthenticated `api.github.com/repos/leetusik/knowledge` → 200) ⇒ clone over **HTTPS** with no credential, then `git remote set-url origin git@github.com:…` so only the *push* path uses the deploy key. Simpler than a sudo + root-only-key SSH clone, same end state.
- **The box can reach github.com over SSH:22** — verified (`Permission denied (publickey)`: TCP + host-key handshake fine, only auth absent). The publish-on-write network path exists.
- **`/opt/knowledge-secrets/` is ALREADY provisioned** on the box: `knowledge_deploy_key` (root, 600), `knowledge_deploy_key.pub`, `known_hosts` — matching the documented shape. Still to confirm: the **`.pub` is registered on GitHub with write access** (`gh repo deploy-key list -R leetusik/knowledge`). `/opt/knowledge` does not exist yet.
- The new vhost **passes a real `nginx -t` against the live tree** (throwaway container, real certs, box untouched) — it is ready to `scp` + `./deploy.sh`.

**🔴 SECURITY FINDING — a live write-capable deploy key's PRIVATE half is untracked in the repo working tree.** Not created by this slice; found by it. `knowledge_deploy_key` + `.pub` sit in the repo root — residue of the **old** SECRETS.md flow (`ssh-keygen -f ./knowledge_deploy_key` writes into the CWD = repo root; "scp to the box then delete the local copy" was never done). There are **two different keypairs and BOTH are registered on GitHub read-write**: `knowledge-api@oci` (id **157264706**, fp `SHA256:+ldq…`) whose private half is **in this tree**, and `knowledge-api@oci-box` (id **157267945**, fp `SHA256:1uQQ…`) whose private half is on the box (fingerprints compared via public halves only). So **`oci` is an ORPHAN** — unused by the box, still write-capable, and one `git add -A` from being committed to a **public** repo.
- **Mitigated in-repo now:** `.gitignore` gained a private-key backstop (`knowledge_deploy_key*`, `*.pem`, `*.key`); verified it ignores both files and catches no tracked file.
- **Operator action still required** (revoking a credential / deleting operator files is out of an executor's remit): `gh repo deploy-key delete 157264706 -R leetusik/knowledge` → `rm -f knowledge_deploy_key knowledge_deploy_key.pub` → confirm only `knowledge-api@oci-box` remains. **Bring-up depends on this being unambiguous:** the authoritative key is **`oci-box`**, the one the box holds and `compose.prod.yml` mounts.
- **Root cause is now fixed at the source:** the rewritten SECRETS.md generates the key **on the box**, so no private half is ever produced on a laptop — this class of residue cannot recur.

**⚠️ Flagged, NOT edited (for the operator / review):** `docs/hi2vi_web/2026-07-02-shared-nginx-explained.md` — a knowledge-base **content** doc — now describes a **superseded** topology (it predates the cutover it proposed). It is not wrong as *history*, only as an operating manual, and `deploy/README.md` now says so. Whether to add a superseded-by note or a follow-up doc is an operator call, out of scope here.

### P8.S5 — E2E acceptance against the LIVE endpoint (delivered — ALL SIX ASSERTIONS PASS)

**The phase's core capability is proven in production.** The box had never actually pushed
a commit before this slice; it has now. One real write to `https://knowledge.hi2vi.com`
travelled the whole chain — **201 `pushed:true` → commit on `main` → Pages deploy green
through the `site_smoke.py` gate → publicly live 65 seconds after the POST returned** →
read back under bearer via hybrid search. No workaround, no papering-over: the acceptance
criteria were met as written.

- **The published artifact (real, public):** <https://leetusik.github.io/knowledge/hi2vi/2026-07-14-the-hi2vi-research-space/> — `hi2vi/2026-07-14-the-hi2vi-research-space.md`, id 7, commit **`383577e`**, an honest inaugural note for the research space (what lands in `docs/hi2vi/`, how documents arrive via the API, the dated-snapshot/`related` convention, how to read/search). Explicitly distinguished from the `docs/hi2vi_web/` engineering explainers. Leak-scanned before sending (no token/box path/hostname/container names).
- **Timings (the publish SLA hi2vi can rely on):** POST returns in **5.8s** (write + index + embed + commit + fetch/rebase/push, all inside the request). Push → Pages deployed: **46s**. POST → publicly readable: **~65s**. So "a note is live about a minute after the agent writes it" is now a measured fact, not a hope.
- **`landing_created:true` + `site_smoke` green = P7.F1 proven end to end.** The first write to a brand-new project auto-created `docs/hi2vi/index.md` in the *same scoped commit*, satisfying the per-project `site/hi2vi/index.html` deploy-gate invariant. The commit contains **exactly three files** (the doc, the auto-landing, the `docs/index.md` Recent bullet), parent = the exact pre-write baseline `4eb85d5` → clean fast-forward, no clobber, no `-A` sweep. The `docs/hi2vi/` bootstrap (intent item 3) is **done, verify-only as the DECOMP predicted**.
- **P8.F2's fix was load-bearing, and its warning was right.** With `openssh-client` in the image the push simply worked (`pushed:true`, no `push_error`). Had it not landed, this same run would have returned an **identical-looking 201** with `pushed:false` and nothing ever reaching Pages. **The E2E's decisive assertion is `pushed:true`, never just the 201** — carry this into any future acceptance of a best-effort path.
- **S1's rebase discipline is clean under real conditions.** Post-push the box clone is `HEAD == origin/main == 383577e`, working tree clean, **no `.git/rebase-merge`/`rebase-apply` leftovers**; the rebase-abort safety net was never needed. Container logs show only `201 → reads → 409 → 401`, no errors.
- **Hybrid search is genuinely live, and writes are searchable immediately.** The new doc is the **#1 hit** on `q=hi2vi research space` with all three signals (`bm25 6.47`, `recency 1.0`, **`vector 0.717`**) — indexed *and embedded* in the same request that wrote it. A query sharing **no keywords** with the doc ("how does the content agent avoid repeating research") also ranks it first → the semantic signal is real, not a BM25 fallback. This is the read path hi2vi needs for topic dedup/grounding (intent item 5): **an agent can search for what it just wrote, in the same session.**
- **Auth boundary confirmed live:** search/read **without** bearer → 401, **wrong** bearer → 401, write without bearer → 401, all with the exact `contract.md` body; `GET /healthz` stays open (documents 6 → 7). `plugin parity` CI also stayed green on the push (F1/F2 parity held).
- **409 dup semantics hold, and cost nothing:** re-POSTing the identical payload → **409** with `id` + `existing_title` (the DB-row shape `contract.md` predicts), and **no second commit** (`main` still `383577e`). The client's documented "409-after-timeout ⇒ already written, read back via `by-path`" rule is safe — the `by-path` read-back is verified too.
- **`contract.md` is now empirically confirmed against production**, not just TestClient: 201 key order, the 409 `detail` object, the 401 body, and the search item/`signals` shapes all match what S4 froze. It is safe for P8.REVIEW to consolidate **verbatim** into `api.md`, and for hi2vi `P15.S4` to code against.
- **Observation for the review (not a defect, not blocking):** publish commits are authored **`kb-api <kb-api@localhost>`** — a placeholder identity now in a *public* repo's history, and every future agent-published doc will carry it. Pre-existing (the `Dockerfile` git identity, untouched by P8). If the operator wants attributable agent commits (e.g. `kb-api@hi2vi.com`), it is a one-line `Dockerfile`/env change — better as a deferred job than a P8 fix.
- **Nothing left dirty:** no `git commit`/`push` run by the executor, no hand-edit of `docs/`/`server/`/`deploy/` (the doc exists *only* via the API, exactly as hi2vi's agent will write it), no box config change or restart. This repo's clone is behind `origin/main` by exactly the 1 commit the API pushed — the orchestrator reconciles. `workflow.py validate` → passed.

## Constraints

- **Operator sign-off precedes implementation** (intent.md): P8.S1+ do not run until the operator approves the proposal above.
- **Local/plugin UX must survive:** reads open-by-default (no token = open), no push locally — both new behaviors are **flag-gated off by default** (`KB_REQUIRE_READ_AUTH`, `KB_GIT_PUSH`); only the box turns them on.
- **No secrets in the repo:** `KB_API_TOKEN`, the push credential, and the Gemini key live only in the box's env/mount, never committed (extends security.md's existing secret-handling rule).
- **Never `git push --force`; never `git add -A`** — the scoped-commit + rebase-onto-remote discipline is the invariant that keeps the box from clobbering `main`.
- **The agent may have no SSH access to the box** — deploy slices produce ready-to-run artifacts + operator-applies-then-validates gates, not live changes.
- **Edge fragility is a documented, accepted operational cost** — knowledge.hi2vi.com rides the same fragile shared edge as hi2vi.com until Option B (D2); the runbook must state the post-changple5-deploy re-apply rule.
- Executor contract: middle slices append their own findings here; durable-truth changes go to **Doc impact** (below), versioned once at P8.REVIEW — never per slice.

## Doc impact

Expected durable-truth changes for **P8.REVIEW** to consolidate into new doc versions (one per doc, capturing the whole phase):

- **`docs/current/api.md`** — hosted deployment section + the **frozen consumer contract** (base URL `https://knowledge.hi2vi.com`, `KNOWLEDGE_API_URL`/`KNOWLEDGE_API_TOKEN`, the new 201 `pushed`/`push_error` fields, read/search now behind bearer on the hosted deployment). This published api.md is the pointer the hi2vi repo references. (Sources: P8.S1, S2, S4.)
- **`docs/current/operations.md`** — production deployment on the OCI box: `compose.prod.yml` (api-only, own-clone, `changple_shared_network`), the `knowledge.hi2vi.com` vhost + TLS, `KB_GIT_PUSH`/`KB_REQUIRE_READ_AUTH`/`KB_PUBLIC_BASE_URL` box env, the edge re-apply rule + cross-repo `apply-to-edge.sh` handoff, publish-on-write flow, bring-up runbook. (Sources: P8.S1, S3, S5.)
- **`docs/current/security.md`** — hosted auth model (reads behind bearer via `KB_REQUIRE_READ_AUTH`; writes already guarded), the push credential (repo-scoped SSH deploy key, box-only), secret provisioning on the box, the deliberate scoped departure from "agent never pushes". (Sources: P8.S1, S2, S4.)
- **`docs/current/architecture.md`** — the hosted-deployment shape (public co-tenant API on the shared edge; box clone + server-side push as the publish-on-write path; local vs hosted flag-gated behavior). Possibly light — confirm at review whether operations/security already carry it. (Sources: P8.S1, S3.)

**Realized by P8.S1 (append to the above at review):**
- api.md — POST 201 and DELETE 200 bodies now always carry `pushed: bool`; `push_error` appears only when a push was attempted and failed; on a successful push `commit_sha` is the **final published HEAD** (a rebase may rewrite the commit), on failed/disabled push it stays the local commit HEAD.
- operations.md — new `KB_GIT_PUSH` flag (default false, box-only true) enables publish-on-write: after the scoped commit the server `git fetch origin main` → rebase → non-force `push origin HEAD:main`, best-effort (a failed push never fails the request; the doc publishes on the next successful push).
- security.md — `KB_GIT_PUSH` is the deliberate, flag-gated departure from "agent never pushes" — off by default so local/plugin never push; only the hosted box (with the deploy-key credential, provisioned in S4) turns it on. Never `--force`, never `add -A`.

**Realized by P8.S2 (append to the above at review):**
- api.md — the read/search surface (`GET /api/documents`, `/api/documents/{id}`, `/api/documents/by-path/{rel_path}`, `/api/search`, `/api/tags`, `/api/projects`) requires `Authorization: Bearer <token>` on the hosted deployment (when `KB_REQUIRE_READ_AUTH=true` and `KB_API_TOKEN` set) → 401 without it; local default leaves reads open. `GET /healthz` is always open. Writes are bearer-guarded regardless of the flag.
- security.md — hosted read-auth model: reads/search go behind the bearer only when **both** `KB_REQUIRE_READ_AUTH=true` **and** `KB_API_TOKEN` are set (`require_read_bearer` delegates to the same `require_bearer` check); flag off (or token unset) leaves reads open, so local/plugin UX is unchanged even with a token set. healthz stays open (count leak immaterial — corpus public on Pages). **No CORS** — server-to-server consumer; the Pages site is browser-only lunr and never calls this API.
- operations.md — new `KB_REQUIRE_READ_AUTH` box-env flag (default false, box sets `true`) that gates the read/search surface behind the existing bearer on the hosted deployment.

**Realized by P8.S3 (append to the above at review):**
- operations.md — production deploy artifacts for `knowledge.hi2vi.com`: `compose.prod.yml` (api-only, no published ports, `build: .`, `container_name: knowledge-api`, box clone `.:/repo`, external `changple_shared_network`, box env with `KB_GIT_PUSH`/`KB_REQUIRE_READ_AUTH`/`KB_PUBLIC_BASE_URL=Pages`/`KB_STARTUP_REINDEX`, secrets via `env_file: .env`, `restart: unless-stopped` + a `/healthz` healthcheck); the `deploy/knowledge.hi2vi.com.conf` nginx vhost (Docker-DNS re-resolution `resolver 127.0.0.11 valid=30s` + variable `proxy_pass` so it survives api recreation, `*.hi2vi.com` wildcard-origin-cert TLS, 80→443, `client_max_body_size 5m`); the `deploy/README.md` bring-up runbook (`COMPOSE_BAKE=false docker compose -f compose.prod.yml up -d --build`, shared-network health probe); and the **edge re-apply rule** — the vhost conf + cert + network attachment are undeclared runtime state on `changple5-nginx-1`, wiped on every changple5 deploy, so **assume knowledge.hi2vi.com is down after any changple5 deploy until the (extended) `apply-to-edge.sh` re-runs** (cross-repo handoff: extend the hi2vi/changple5 `apply-to-edge.sh` to also restore knowledge's conf + cert).
- security.md — the box push credential is a **repo-scoped SSH deploy key mounted read-only** (`/opt/knowledge-secrets:/run/secrets:ro`, key chmod 600) with `GIT_SSH_COMMAND` pinning identity + host key (`-i … -o IdentitiesOnly=yes -o UserKnownHostsFile=… -o StrictHostKeyChecking=yes`, pinned `known_hosts` over `accept-new` to close the first-push TOFU MITM window); secrets (`KB_API_TOKEN`, `GOOGLE_API_KEY`) live only in the box's gitignored `.env` (never committed — `.env` added to `.gitignore`), never in the repo.

**Realized by P8.F1 (append to the above at review):**
- qa.md/operations.md — parity guard lesson: local slice validation for shipped-payload files (`server/*`, `tests/*`) must include `scripts/plugin_parity.py`, not just pytest — parity only runs in CI (`plugin-ci.yml`) on push, so S1/S2's local pytest gate missed the drift they introduced. Plugin version bumped 0.1.0 → 0.2.0 (`plugin/.claude-plugin/plugin.json` + `plugin/skills/setup/SKILL.md`'s scaffold-marker `plugin_version`) with the S1/S2 payload sync (new backward-compatible server capability + response fields ⇒ minor bump).

**Realized by P8.S4 (append to the above at review):**
- api.md — **consolidate the frozen consumer contract VERBATIM from [`contract.md`](contract.md)** (everything below its horizontal rule) as a new `## Hosted deployment + frozen contract` section: base URL `https://knowledge.hi2vi.com`, `KNOWLEDGE_API_URL`/`KNOWLEDGE_API_TOKEN`, bearer required on **all** `/api/*` on the hosted box (healthz open), the full `POST /api/documents` request/response contract (201 with `pushed`/`push_error` + `commit_sha`=published-HEAD semantics, 409/422/401 verified shapes), the read/search shapes, and the operational semantics (publish flow, retry-on-409/never-retry-on-`pushed:false`, single-daily-writer, hybrid/BM25 fallback). Shapes are TestClient-verified against `server/main.py` — carry them as-is. This published api.md is the pointer the hi2vi repo references.
- operations.md — the secrets/DNS **provisioning runbook** `deploy/SECRETS.md` (generate + register: `KB_API_TOKEN`, SSH deploy key + GitHub deploy-key registration, Cloudflare proxied `knowledge` record + `*.hi2vi.com` cert-coverage check w/ per-host fallback, optional `GOOGLE_API_KEY`) and its **handoff checklist**, feeding the `deploy/README.md` bring-up runbook (README now links SECRETS.md instead of the `P8.S4` slice id). SECRETS.md = "produce + register", README = "place + bring up".
- security.md — secret provisioning as **operator-only, box-only actions**: the bearer token (`openssl rand -hex 32`, shared with hi2vi as `KNOWLEDGE_API_TOKEN`), the repo-scoped SSH deploy key registered on GitHub **with write access** (private half box-only at `/opt/knowledge-secrets/`, local copy deleted after placement), origin pinned to the SSH URL; the optional Gemini key. Reinforces the "no secret values in the repo — names + placement paths only" rule (extends S3's security note; no new code).

**Realized by P8.F2 (append to the above at review):**

> ⚠️ **P8.F2 SUPERSEDES PARTS OF THE P8.S3 BLOCK ABOVE.** S3's doc-impact was written against the old shared edge and is now **factually wrong**. When consolidating `operations.md`, **do not carry** any of: `changple5-nginx-1`, the `deploy/knowledge.hi2vi.com.conf` filename (renamed to `deploy/knowledge.conf`), the "vhost/cert/network are undeclared runtime state wiped on every changple5 deploy" fragility claim, the "assume knowledge.hi2vi.com is DOWN after any changple5 deploy" operational rule, or the cross-repo `apply-to-edge.sh` handoff. **None of them are true any more.** Take the edge story from THIS block. (S3's compose/env/secrets facts are still correct.)

- **operations.md** — **the real production edge.** The box runs a **dedicated edge**: compose project `edge` at `/home/opc/edge`, container `edge-nginx` (`nginx:1.27-alpine`), sole owner of `:80`/`:443`, on `changple_shared_network`. Its `conf.d/` + `certs/` are **read-only host bind mounts**, so the edge config is **declarative host state** and the apply path is a **host file drop + `cd /home/opc/edge && ./deploy.sh`** (hard `nginx -t` gate inside the running container → graceful `nginx -s reload`, **never a recreate**; `./validate.sh` is the optional pre-gate; **never `docker cp`/`exec`/`up`/`restart` the edge**). **Because the config is declarative host state on read-only mounts, a co-tenant (changple5) deploy CANNOT wipe `knowledge.hi2vi.com` — the old re-apply rule and the `apply-to-edge.sh` handoff are OBSOLETE and deleted.** This is the D2 / Option-B end state, now live. The vhost ships as **`deploy/knowledge.conf`** (renamed from `knowledge.hi2vi.com.conf`) and lands at `/home/opc/edge/conf.d/knowledge.conf`. **TLS needs no provisioning**: the edge's `hi2vi.crt` is a Cloudflare Origin CA cert with SANs `*.hi2vi.com, hi2vi.com` (valid to 2041), so the wildcard already covers `knowledge.hi2vi.com` (settles the DECOMP "TLS cert mechanism" open question: **wildcard**). Edge-tree rules a vhost must obey (they break the *whole* edge, not just one site): the `conf.d/` tree is tested/reloaded **as a unit**; `limit_req_zone` names are **global** across it (ours declares none); **never `default_server`** (`00-default.conf` owns the catch-alls → `444`); **no IPv6 `listen`** (no sibling listens on v6, so adding it would silently make that vhost the v6 default server).
- **operations.md** — **`openssh-client` is load-bearing for publish-on-write.** The image installs git with `--no-install-recommends`, which drops `ssh` (only a *Recommends* of git), so the container **could not push at all** over its SSH remote — and since push is best-effort, that failed **silently** (still 201, `pushed:false` + `push_error`, nothing reaching Pages). `openssh-client` is now in the `Dockerfile` (and the plugin template; plugin **0.2.1**). **Bring-up must assert it** (`docker compose -f compose.prod.yml exec api command -v ssh` → `/usr/bin/ssh`), and acceptance must check **`pushed:true`**, never just the 201. General rule: *a best-effort code path can hide a total infrastructure failure — assert the capability at bring-up.*
- **operations.md** — **box bring-up facts:** the clone must be **`opc`-owned** (`docker compose` reads `compose.prod.yml` + `.env` client-side as the invoking user; a root-owned 600 `.env` is unreadable and the bring-up fails), with `.env` mode 600 owned by `opc`; the repo is public, so clone over **HTTPS** and then point `origin` at the **SSH** URL (only the push path needs the deploy key); the container runs as root and writes git objects into the clone as root (host-side `git` there may need `sudo`).
- **security.md** — **private keys are gitignored as a backstop** (`knowledge_deploy_key*`, `*.pem`, `*.key` in `.gitignore`). Motivated by a real incident found in P8.F2: the *old* "generate the key locally (`ssh-keygen -f ./knowledge_deploy_key` → the repo root!), scp it to the box, delete the local copy" flow left a **live, write-capable GitHub deploy key's private half untracked in this public repo's working tree**. Two lessons, both now enforced: **generate credentials where they will live** (on the box), and **never rely on a "remember to delete the local copy" step**. The `.gitignore` guard is a backstop, not a place to put a key.
- **security.md** — **secrets never transit.** The push credential (SSH deploy key) is **generated on the box**, in its final location (`/opt/knowledge-secrets/knowledge_deploy_key`, root, 600, mounted read-only into the container) — the private half **never leaves the box**; only the **public** half moves, registered on `leetusik/knowledge` as a **deploy key with write access** (`gh repo deploy-key add … --allow-write`). This replaces S4's "generate locally, scp to the box, delete the local copy" flow. Likewise **`KB_API_TOKEN` is generated in place** into the box's gitignored `/opt/knowledge/.env` (an unquoted heredoc expands `$(openssl rand -hex 32)` as the file is written), so the value never appears in a chat or terminal transcript; it is read back only on demand (`sudo grep '^KB_API_TOKEN=' /opt/knowledge/.env`). Repo still carries **names and paths only**.

**Realized by P8.S5 (append to the above at review):**

- **qa.md** — **the hosted E2E acceptance procedure**, run for real against `https://knowledge.hi2vi.com` (2026-07-14) and green on all six assertions: (1) `POST /api/documents` → 201 **with `pushed:true`**, `landing_created:true`, a `commit_sha`, and a Pages `url`; (2) the `commit_sha` **is** `main`'s HEAD on GitHub (verify against the remote, not the box) and its tree holds exactly the doc + the auto-created `docs/<project>/index.md` + the `docs/index.md` Recent bullet; (3) the push's `pages.yml` run concludes **success** (its `site_smoke.py` step is the per-project landing gate) and `plugin-ci.yml` stays green; (4) the doc is live at `url` (~1 min); (5) `GET /api/search` **with** the bearer finds it (expect `mode:"hybrid"`; the doc is embedded in the same request that wrote it, so it is searchable immediately) and **without** the bearer → 401; (6) a duplicate POST → **409** with no second commit. **The central lesson: assert `pushed:true`, never just the 201.** Push is best-effort by design, so a totally broken publish chain (e.g. the missing `openssh-client` P8.F2 caught) returns an *identical-looking* 201 with `pushed:false` + a `push_error` and nothing ever reaches Pages. **Generalize: when a capability is best-effort, acceptance must assert the capability, not the request's status code.**
- **operations.md** — **`knowledge.hi2vi.com` is validated end-to-end in production.** The full publish-on-write chain is proven live: write → index+embed → scoped commit → fetch/rebase/non-force push to `main` → GitHub Pages deploy (through the `site_smoke.py` gate) → publicly readable. **Measured timings (the SLA the hi2vi agent can rely on):** the POST returns in **~6s** (all of the above happens inside the request), the Pages deploy completes in **~46s**, and the document is **publicly live ~65s after the POST returns**. Post-push the box clone is clean (`HEAD == origin/main`, no rebase leftovers). Hybrid search (`mode:"hybrid"`, Gemini vector signal live) and bearer-gated reads (401 without/with a wrong token; `healthz` open) are confirmed on the box. The first write to a **new** project auto-creates its `docs/<project>/index.md` in the same commit, so it cannot break the site build — verified, not assumed.

Contract file: [`works/phases/active/P8/contract.md`](contract.md) — **consolidate verbatim into api.md at review.** Its shapes are now **confirmed against the live production box** (P8.S5), not just the TestClient: the 201 key order, the 409 `detail` object (with `id`/`existing_title`), the 401 body, and the search item/`signals` shapes all match. Safe to carry verbatim; safe for hi2vi `P15.S4` to code against.

### P8.REVIEW — phase review (verdict: **pass**; 6 doc versions consolidated)

**Validation, all slices together:** pytest **65 passed**; `plugin_parity.py` **PASS**;
`workflow.py validate` **PASS**; and — added beyond the plan — `docker compose run --rm kb build`
+ `site_smoke.py` **PASS** *after* writing the doc versions, because `docs/current/` is published
and the review's own output rides the next Pages deploy.

**Live production spot-checks (read-only), all green:** `healthz` **200** (open, `documents:7`,
`db:ok`); `GET /api/search` and `GET /api/documents` **without** the bearer → **401** (read auth
is genuinely on in prod); S5's doc **and** the auto-created `docs/hi2vi/` landing both **200** on
Pages; and `383577e` confirmed an **ancestor of `origin/main`** with a 3-file tree. Publish-on-write
is real, not just implemented.

- **Two planned checks were NOT run** — the authed `GET /api/search` (`mode:hybrid`) and the box
  clone's cleanliness both need SSH into the box to read `KB_API_TOKEN`, and the executor's
  **permission system denied box access + token extraction**. Not worked around. Both were verified
  live at S5 with evidence, and the un-authed 401 exercises the same route as the authed 200. No
  defect signal — but stated plainly rather than papered over.
- **Gotcha for anyone re-running the plan's box check:** `HEAD == origin/main` on the box is
  **stale as an assertion** now — the orchestrator has pushed P8's slice commits since S5, so the
  box clone is legitimately *behind*. Behind is fine and self-healing (fetch+rebase before every
  push). Only a **dirty tree** would break the next push; that is the thing worth checking.

**Judged against `intent.md`: all six deliverables met.** Endpoint live; auth enforced on writes
*and* reads; `docs/hi2vi/` bootstrapped by the auto-landing inside the same scoped commit (so it
could never break the deploy gate); publish-on-write proven end-to-end in production (~65s POST →
publicly live); read/search exposed under the same bearer, with a doc embedded **in the same
request that wrote it** (so the agent can search what it just wrote — exactly intent point 5); and
the frozen contract now **published in `api.md` v0005**, which is the pointer hi2vi `P15.S4`
references.

**Verified in source, not merely trusted from `result.md`:** both new flags default **false**
(`git_push_enabled` / `require_read_auth_enabled`, truthy-parse — the deliberate inversion), so
local/plugin keeps open reads + no pushing and a set token still guards **writes only**;
`gitops.push()` has **no `--force` and no `add -A`** anywhere; six read routes carry
`Depends(require_read_bearer)`; `openssh-client` is in **both** Dockerfiles. Also confirmed **D3 is
truly remediated**: no `knowledge_deploy_key*` in the tree, and `git ls-files` shows no key
material, `.pem`/`.key`, or `.env` tracked.

**Doc versions (one per doc, whole-phase):** `api` **v0005** (frozen contract **verbatim** from
`contract.md`), `operations` **v0009** (the real declarative edge + publish flow + `openssh-client`),
`security` **v0004** (hosted auth, box-only deploy key, the narrowed never-push rule),
`qa` **v0006** (the hosted E2E + its two lessons), `architecture` **v0007** (two deployments, one
codebase), `decisions` **v0009** (seven ADRs + two Superseded entries).

- **The superseded S3 edge facts were kept out**, as instructed: the published set contains no
  `changple5-nginx-1`, no `apply-to-edge.sh`, no old conf filename, and no "wiped by every
  co-tenant deploy" rule. The only `docker cp` mentions are the F2 **prohibition**.
- **Publish safety re-scanned:** no token, no key material, no origin IP, no `/Users/` path. Box
  paths appear only as facts already committed in `deploy/`; the *mechanics* are referenced to
  `deploy/README.md` / `deploy/SECRETS.md` rather than duplicated, so the durable docs cannot drift
  against the runbooks.

**The phase's most consequential decision, for the record:** making read auth an explicit
**flag** rather than "a set token implies closed reads". The convenient version would have
silently broken **every existing local and plugin user who had set a token** (their token guards
writes; their reads are open by design). Same instinct behind `KB_GIT_PUSH` defaulting off.
Default-off *is* the compatibility contract — it is what let a public, authenticated, self-publishing
deployment ship without touching anyone else's experience.

**Process lesson the phase paid twice for** (now in operations + qa): S3/S4 authored deploy
artifacts against a *knowledge-base explainer doc* instead of the live box, and F2 found the
targeted edge no longer existed **and** that the image could not push at all — a failure that was
totally silent because push is best-effort. **Assert the capability, never the status code**; and
check artifacts against reality, not against a doc that describes the world as it was.

## Open Questions

_All four settled during the phase; kept for provenance._

- **TLS cert mechanism** — **SETTLED (F2): wildcard.** The edge's Cloudflare Origin CA cert carries `*.hi2vi.com` (valid to 2041), so `knowledge.hi2vi.com` needed **no cert provisioning**; the per-host branch was deleted from the vhost as dead code.
- **Push credential form** — **SETTLED (S3/S4/F2): SSH deploy key**, repo-scoped, write access, **generated on the box** so the private half never leaves it. PAT remains a fallback (`push()` is URL-agnostic).
- **Gemini key at launch** — **SETTLED: provisioned.** The box runs `mode:"hybrid"` with a live vector signal (S5 measured `vector 0.717` on the inaugural doc, and a zero-keyword-overlap query still ranked it first).
- **DELETE push scope** — **SETTLED (S1): DELETE pushes too**, same flag, same best-effort semantics — parity for the reverse path.
