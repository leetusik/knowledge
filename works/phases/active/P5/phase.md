...
| `P5.S5` *(added 2026-07-11 — operator co-work, intent amendment)* | Design co-work — operator designs targets in Claude Design; agent syncs (DesignSync) & integrates as delivered; holds the design-target list | implementation | medium | 1.5 | P5.S1 |
...
- **P5.S5 Design co-work (medium, added 2026-07-11 by intent amendment):** operator co-work slice at order 1.5 — sits `pending` while the operator designs each target in Claude Design; on each delivery the agent syncs it (DesignSync `list_projects`/`list_files`/`get_file`), integrates it into the site replacing that piece of the S1 baseline, validates (docker build + marker-contract checks), commits, and returns the slice to `pending` for the next target. The design-target list and per-target checklist live in `slices/P5.S5/plan.md`. **Downstream re-scope:** P5.S2 keeps the UX/marker-contract mechanics and integrates the delivered landing design (target 8) rather than inventing a layout; P5.S3 keeps the CJK search engineering and consumes target 10's search-UI styling; P5.S4 unchanged. Visual design decisions now come from the operator's deliveries, not from agent taste.
