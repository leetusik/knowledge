...
doc_id: frontend
version: v0016
created_at: 2026-08-07T17:51:28+09:00
source: P25.REVIEW
summary: P25.F4 the explainer height handshake documented in full and made hydration-race-proof: a child-ward kb-explainer-request re-post, plus the F3 share hint conditions
previous: v0015_p25_pretty_public_routes_org_project_slug_org_graph_the_resolvedocument_bff_seam_legacy_307_redirects_and_the_dashboard_public_url_panel
---
...
- **iframe chrome (`explainer.css`, co-located).** `.kb-explainer` mirrors the `.kb-graph`
  ...
  **Superseded:** the frame *is* content-sized today, by the `postMessage` handshake below —
  see *The explainer height handshake (`web/`)*. The P16 fixed-height rule survives only as
  `explainer.css`'s fallback, i.e. as the failure mode when no height message arrives.
...
## The explainer height handshake (`web/`)

The explainer frame is **content-sized**, so the PAGE scrolls and the frame never does. The parent cannot measure it — `sandbox="allow-scripts"` without `allow-same-origin` (P16 pinned decision 1) makes the framed document an **opaque origin** — so the height is reported outward by the child over `postMessage`. Two halves: the child reporter is source text in `lib/explainer-height.ts`, **injected by the BFF raw routes** (`/api/documents/[id]/raw` and `/api/documents/[id]/versions/[v]/raw`) rather than authored into the `explain` template, so it fixes every explainer already saved and leaves the three parity-guarded `SKILL.md` copies untouched; the parent half is the `ExplainerFrame` client island, which `document-view.tsx` and the past-version page both render — so one fix reaches every article view (id, pretty, anonymous, versions) **by construction**.

- **Three messages.** Parent-ward: `kb-explainer-height` (the document's content height) and `kb-explainer-anchor` (a "Contents" link's target offset — a content-height frame has nothing left to scroll, so the parent scrolls the real page instead). Child-ward, since **P25.F4**: `kb-explainer-request` — *"re-send your height"*.
- **Why the request exists (the hydration race, P25.F4).** The frame's `src` is in the SSR HTML, so the child can load and post its one height message **before** the parent island hydrates and attaches its listener — and because the reporter dedupes (it re-posts only on a real ≥2px change), that missed first message was never repeated: the frame kept `explainer.css`'s fallback height forever and scrolled internally, until an unrelated window resize happened to un-wedge it. Reproduced deterministically on a local production build under `Emulation.setCPUThrottlingRate ≥ 4`, on **every** article view — not just the member one. The member branch merely loses the race more often (five client islands vs `PublicShell`'s zero), which is why the anonymous view looked fine. *Never conclude a client-island bug is branch-specific without re-testing under throttle.*
- **The fix is two-sided, and covers both orderings.** The parent posts `{type:"kb-explainer-request"}` into `frame.contentWindow` (i) inside the `useEffect`, immediately after `addEventListener("message", …)` — covering a frame that had already finished loading — and (ii) from the iframe's `onLoad` — covering one that had not, since React attaches `onLoad` during the hydration commit, *before* effects run. Between them there is no gap.
- **`targetOrigin: "*"` is mandatory, not laziness.** An opaque origin can never be named by a concrete origin string. Nothing leaks: the message carries no data and is delivered to that one frame's window only.
- **The request is the ONLY trigger that bypasses the dedupe.** The child's handler is sender-checked (`e.source !== window.parent` → ignore) and shape-checked, then resets **`last` only, never `budget`** — a runaway document stays latched. Load, resize, `ResizeObserver` and font events still need a real change, which is what keeps a parent→child→parent feedback loop impossible (verified live: exactly two height messages arrive, and the applied height is identical at t+4s and t+8s).
- **Every trust safeguard is verbatim.** `sandbox="allow-scripts"` without `allow-same-origin`, the `event.source === frame.contentWindow` identity check (**never** `event.origin` — it is the literal string `"null"` for an opaque origin), the message shape check, and the `[120, 40000]` clamp. No CSP, relay, route, layout or CSS change was needed by P25.F4.
- **The child's own measurement rules still hold** and are worth re-reading before touching the reporter: it measures `body`'s box (never `documentElement.scrollHeight`, which is floored by the frame's height and would ratchet), adds the collapsed body margins back explicitly, and debounces with `setTimeout` — **never `requestAnimationFrame`**, which Chrome does not run in an opaque-origin frame at all.
...
- **The unclaimed-name hint beside the copy affordances (P25.F3).** On the id document page's **member** branch and in the member graph header, a viewer whose own org has no slug gets one sentence from `content/share.ts`'s `SHARE.claimHint` (split `prefix` / `linkLabel` / `suffix` / `href` because the middle is an inline `<Link>` to `/dashboard`; every character still lives in the content module). No new component and **no new CSS**: it reuses `--kb-hint` + `--kb-accent-strong` with `underline underline-offset-2`, the treatment `version-history.tsx` already uses, so it themes automatically. The graph header stacks it under the button in a `flex-col items-end` column because that header is a `justify-between` title/action bar. **The condition is the VIEWER's own `identity.tenant?.slug`, never `canonical_path` alone** — a claimed slug with a `null` path is the P25.F1 superseded case, which is nobody's to fix. The document page needs a **second clause** (`doc.canonical_path === null`) as an ownership proxy, because that member branch is *not* own-org only (`_resolve_readable_doc`'s public-project fallback lets a cross-org member read another org's public doc) and `KbDocument` carries no `tenant_id`. **If a future slice needs exact ownership on a share surface, add a backend field — do not infer it in TypeScript.**
- **Validation.** `npm run lint` / `typecheck` / `build` / `test` (**16 files / 88 tests** as of P25.F4; 15 / 85 through F3) all green; the build's route table carries `/[org]/[project]/[slug]` and `/[org]/graph` in the order above, and neither F3 nor F4 moved a route.
