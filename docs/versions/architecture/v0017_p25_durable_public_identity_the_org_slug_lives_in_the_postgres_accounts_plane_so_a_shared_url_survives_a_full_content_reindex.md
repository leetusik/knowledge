...
doc_id: architecture
version: v0017
created_at: 2026-08-07T16:00:52+09:00
source: P25.REVIEW
summary: P25 durable public identity: the org slug lives in the Postgres accounts plane so a shared URL survives a full content reindex
previous: v0016_p24_publish_moves_behind_the_response_a_single_in-process_fifo_worker_thread_runs_push_and_embed_after_the_201_while_re-taking_the_one_write_lock_with_every_git_subprocess_bounded
---
...
A single `POST /api/documents`. Steps 1–5 run **in-request**, under one in-process write lock, and are **entirely local** (no network); step 6 runs **after the response** (P24):
...
4. upsert the DB row — **the document is durably saved at this point**;
5. make a **scoped** git commit — `git add` only the touched paths (**never `-A`**);
6. → **201 goes out here** ← then, on the publish worker: **(P8, hosted only)** publish it — fetch → rebase onto `origin/main` → **non-force** push — and embed it for semantic search.

**(P24) The response/publish split is the load-bearing boundary.** Everything the caller waits for is local disk + SQLite; every network round-trip (GitHub over SSH, Gemini over HTTPS) sits behind the response on a single FIFO worker thread. Before P24 steps 6 ran *inside* the request — and inside the write lock — with no subprocess timeout, so a slow remote spent the caller's deadline on work whose result never appeared in the response: the client timed out on a write that had already landed. The worker re-takes the same one write lock (a rebase mutates the tree), so tasks are always submitted from **outside** the critical section, and every git subprocess is bounded (`KB_GIT_TIMEOUT_S`, default 60 s) so a hung network can never hold that lock indefinitely.

A failed commit **never rolls back** the file/DB (`committed:false`); `docs/` stays canonical and `POST /api/reindex` reconciles any drift (manual edits, API-down fallback writes, git resets). A failed **push** likewise never fails the write — and since P24 it is not even *visible* to the caller (the response is long gone): it is logged on the box, and the commit publishes on the next successful push. The corollary for every client: **a lost or timed-out write response is not evidence of failure** — verify by reading the document back, never blind-retry.
...
| After the scoped commit | **never pushes** | fetch → rebase → non-force push (`KB_GIT_PUSH=true`), **after the response** (P24) |
...
- **The write path became the publish path.** Hosted, a single `POST /api/documents` carries a
  document all the way from JSON to a publicly readable page: file + landing + Recent bullet +
  DB row + commit, then embedding + push. No operator action, no cron, no external queue.
  **(P24) the chain is now split in two by the response:** the durable, caller-visible half runs
  in-request inside the write lock; the publish half (push + embed) runs on **one in-process FIFO
  worker thread** that re-takes the same lock. This makes the single-writer invariant *more*
  load-bearing, not less — it is what lets a git rebase run safely on a background thread of a
  process that is also serving writes — and it splits the two failure domains cleanly: the save
  is synchronous and reported, the publish is asynchronous and logged.
...
## Accounts v2: user → org → project (P18)

P18 restructures the P10 accounts plane into an explicit **user → org → project** model, entirely **inside the existing two-plane process** — no new service, container, port, edge route, or Postgres instance. Three architectural facts:

- **The tenant row *is* the org — no rename.** The product now says "org," but the `tenants` table and the `tenant_id` that threads through the content plane (`documents.tenant_id`, `usage_events`, the resolver, seed, MCP, the web BFF) are **unchanged**. A `tenants → orgs` rename would be a large mechanical diff across both planes with **prod-regression risk and zero behavior change**, so it was consciously rejected (see decisions). "Org" is a product-facing naming layer over the same identity; org **creation** + member **invites**/roles are deferred (D14).
- **Credentials are tenant-scoped at the schema level, with an org-vs-project split the hot auth path honors.** `0003` (see data) puts a `tenant_id` on every credential and makes `project_id` nullable. An **org-level** key (`project_id NULL`) authorizes the whole tenant/org; a **project-bound** key keeps its `project_id` for attribution + the vanished-project existence guard. The `/api/*` resolver reads the credential's own `tenant_id` directly — so org-level keys realize, in the schema and the resolver, the tenant-wide enforcement that P10 already had de-facto (the write path never checked a doc's `project` against the credential). This is a schema/wording change, **not a trust-boundary shift** (see security).
- **Get-or-create lives in an async dependency ahead of the sync write handler.** `POST /api/documents` must ensure a Postgres `projects` row exists for `(tenant_id, body.project)`, but the sync handler holds `WRITE_LOCK` over the sqlite critical section and cannot `await` Postgres. So an **async `ensure_registry_project` FastAPI dependency** runs the race-safe get-or-create (against the new `UNIQUE(tenant_id, name)`) **before** the handler — FastAPI parses the body once and caches the resolver, so the dep runs exactly once. This is the seam that lets a Postgres write precede the sqlite-locked write. The single-worker invariant is untouched; the async accounts plane still never touches `WRITE_LOCK`.

## Durable public identity: what a shared URL may be made of (P25)

P25 adds no service, container, port, edge route, or plane. What it adds is a **rule about which plane a public identifier may come from**, and one column to satisfy it.

- **The problem is the two-plane split, not the URL.** `documents.id` is a rowid in the **disposable** SQLite content plane, which is rebuilt from files on boot and re-derived by a full reindex — the store's own code already treats it that way (a document's durable identity is its `rel_path`, and the version-history table is deliberately not FK'd to it). So `/documents/25` was never a durable address; it merely worked because the box had not been reindexed from scratch. A URL that a stranger keeps in a bookmark must not key off it.
- **The rule.** A public URL is composed **only of durable parts**: the org slug (Postgres accounts plane — see **data**), the project name, and the doc slug. The latter two are already inside the content plane's durable identity `rel_path = "<project>/<date>-<slug>.<ext>"`, and the project name is the same string on both planes (it is a directory name, so it is URL-safe by construction and needs no separate project slug). Nothing new is stored in the content plane and there is no cross-plane mapping table.
- **Why the slug goes in Postgres.** It is the only one of the three parts with no durable home: it names the **org**, which exists only in the accounts plane. Putting it there is what makes a share link survive `rm kb.sqlite3` + reindex. It is also the reason a slug is **globally** unique while every other constraint in that schema is tenant-scoped — it is a public namespace, not a tenant-private one.
- **Composed once, in the backend.** The pretty path is emitted by the API as an additive `canonical_path` on the detail reads and on the graph response, and every consumer (the web pages, the copy-link surfaces, the legacy redirects) **consumes** it rather than re-deriving it in TypeScript. One assembly site means one place where the rule above can be violated, and one place to add the round-trip guard that keeps a dateless address honest (see **api**/**backend**).
- **Trust boundary unchanged.** P25 introduces no new visibility semantics: the slug resolver lands on the same extracted public-read predicate as the P19 anonymous reads, legacy mode (`DATABASE_URL` unset) still never touches the accounts plane, and every miss on the new surfaces is the same indistinguishable 404. The addition is an *address space*, not an *access path*.

## Boundaries / Constraints

- **Single-writer**: one uvicorn worker + one in-process lock — never scale workers (WAL gives read concurrency). P8 made this stricter, not looser: the git push lives inside that lock. P10 keeps this exactly: the async Postgres plane sits alongside but never touches `WRITE_LOCK`. **P24 keeps it too, from a second thread**: the publish worker takes the *same single* lock, so writers are still serialized — which means a publish task must be **submitted from outside the critical section** (the lock is non-reentrant) and every git subprocess it runs must be **time-bounded** (`KB_GIT_TIMEOUT_S`), or a hung network would hold the process's only write lock forever.
- **(P24) No network I/O before a response.** Anything that talks to a remote (GitHub, Gemini) belongs on the publish worker, never on the pre-response path. This is a *product* constraint as much as an architectural one: a caller's timeout budget must never be spent on work whose result it cannot see.
