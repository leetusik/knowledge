...
doc_id: operations
version: v0014
created_at: 2026-07-18T03:17:45+09:00
source: P13.REVIEW
summary: P13 knowledge CLI install + edge routes /auth+/app + hosted accounts-plane cutover runbook + auth throttle knobs
previous: v0013_p12_web-app_local_build_run_stub_kb_api_base_url_session_secret_full_deploy_p14
---
...
- **Deferred to P14:** the app's Dockerfile / compose service / edge vhost (behind the shared edge, `output: "standalone"` like hi2vi_web), the public landing page, and the operator's live-app visual acceptance. P12 changes nothing in the current production box (the api + mkdocs-site two-service stack is untouched).
...
- **The web app's production deploy is P14** (P12) — P12 ships `pnpm dev` + `output: "standalone"` only; do not add its Dockerfile / compose / edge vhost before P14.
