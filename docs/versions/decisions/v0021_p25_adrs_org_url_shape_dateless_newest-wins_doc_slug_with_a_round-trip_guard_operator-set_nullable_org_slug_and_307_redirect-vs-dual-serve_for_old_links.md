...
doc_id: decisions
version: v0021
created_at: 2026-08-07T16:05:29+09:00
source: P25.REVIEW
summary: P25 ADRs: /@org URL shape, dateless newest-wins doc slug with a round-trip guard, operator-set nullable org slug, and 307 redirect-vs-dual-serve for old links
previous: v0020_p23_adrs_version_history_on_disk_because_the_content_db_is_disposable_identity-keyed_derived_tables_overwrite_becomes_non-destructive_identity_immutable_across_a_version_chain
---
...
- Decision: adopt the revised interaction model. It **supersedes** two S0 locks:
  - **Label Strategy A → A′.** S0 locked "doc titles always on". A′ makes the idle map
    **marks-only (quiet)**: a node's title reveals on hover/selection, doc titles fade
    up past ~110% zoom, and tag labels stay on-demand. *(Narrowed at P22 to the public
    mkdocs map — the app's ported renderer is selection-driven; see D-P22-1.)*
...
### Fix the broken hero install line with a live curl installer, not a PyPI publish (D-P20-1)

- Date: 2026-07-22
- Status: accepted (P20.S1)
- Context: The hero's `uv tool install knowledge-cli` was live-broken — the package was never published to PyPI (D-P13-1: git-subdir distribution, "PyPI stays a later, separate call"). The operator hit the failure live and left the choice open ("make it work or fix the command").
- Decision: Ship a **live curl installer** at `web/public/install.sh` (served at `/install.sh`, zero infra) that bootstraps `uv` if missing then runs the canonical `uv tool install --reinstall git+…#subdirectory=cli`; the hero line becomes `curl -fsSL https://knowledge.hi2vi.com/install.sh | bash`. **PyPI stays out of scope** — D-P13-1 is unchanged (still needs an operator-owned PyPI account). The raw `git+` line stays the canonical/manual fallback in `cli/README.md` + `guide.py`. Windows `install.ps1` is a POSIX-only deferral (**D20**).
- Consequences: The installer delivers whatever is on GitHub main, so S1's CLI changes reach installer users only after the operator-gated push (S4); `--reinstall` makes re-runs idempotent + upgrade-safe.
- Source: P20.S1 `result.md`; P20 `phase.md`.

### Relax the CLI reuse-gate: reuse the org key across projects (D-P20-2, resolves D16)

- Date: 2026-07-22
- Status: accepted (P20.S1); resolves deferred **D16**
- Context: `knowledge init --project other` re-minted a second org key on a recorded-project change, even though org keys are project-agnostic (`project_id NULL`, P18). P18 (D-P18) flagged this re-mint as a candidate follow-up (D16).
- Decision: Drop `same_project` from the reuse condition (→ `if api.get("token") and same_service and not args.new_key`), so a recorded org key is **reused across projects**; still re-mint on `--new-key`, still gate on same-service. Preserve the "absent = unknown, never mismatched" invariant + the show-once property. A pre-P18 project-bound key fires a one-time note when the requested project differs.
- Consequences: One org key now serves every project from the CLI (matching "one key, all repos"); resolves D16.
- Source: P20.S1 `result.md`; P20 `phase.md`.

### Password honesty = depict the prompt + a web-login line; no password generator (D-P20-3)

- Date: 2026-07-22
- Status: accepted (P20.S1)
- Context: The hero never showed the password step, so a user couldn't tell how to log into the web UI (the operator suggested generating a random password). The flow already prompts via getpass; there is no `--password` flag by design.
- Decision: **Depict the real prompt** in the hero and add one base_url-aware line to `knowledge init` success output — `web login: {base_url}/login (same email + password)` — making the shared accounts plane obvious. **No password generator, no signup-flow/backend change.**
- Consequences: The web-login connection is now explicit from the terminal; no new credential surface.
- Source: P20.S1 `result.md`; P20 `phase.md`.

### Publish the explain skill as a served `/SKILL.md` under a third, full-file parity copy (D-P20-4)

- Date: 2026-07-22
- Status: accepted (P20.S3)
- Context: The landing must offer the canonical explain skill copyable + downloadable. The skill is byte-parity-guarded across `plugin/` (canonical) and `.agents/` (body-only) by `scripts/skills_parity.py`; a landing copy must never fork.
- Decision: Serve `web/public/SKILL.md` at `/SKILL.md` (standalone image + nginx catch-all, zero infra) as a **third** parity copy `WEB`, held to a **stricter full-file** byte match (frontmatter included — a straight publish, not the body-only derivation the `.agents/` copy uses); missing/mismatch FAILs in CI. Download = `<a download>`; **Copy fetches the served file** (never a pasted fork).
- Consequences: The landing artifact can never drift from the canonical skill; `plugin-ci.yml` needs no change (it already runs the script).
- Source: P20.S3 `result.md`; P20 `phase.md`.

### Hero copy is a content fix (no design round); new landing sections go through design round 02 (D-P20-5)

- Date: 2026-07-22
- Status: accepted (P20.S2/S3)
- Context: The hero fix changes designed-component *content*; the env-var quickstart + skill-on-landing are genuinely new landing sections.
- Decision: Treat the hero terminal copy change as a **content/copy fix inside the already-designed hero** — no design round; note the departure in a code comment; `web/design/rounds/01-*` stays read-only. The two new sections went through the **design-cowork gate (round 02)**, landed AS-IS, then were implemented in a separate slice. **No token delta** (bronze `#c8a15e` stays a literal); **D10 resolved** (the two mid-feature ledes quoted verbatim from build-prompt §D10).
- Consequences: Round 02 record kept read-only under `web/design/rounds/02-onboarding/`; deferred **D10** closed as done.
- Source: P20.S2/S3 `result.md`; P20 `phase.md`.

### The app graph's labels leave the port: selection-driven, not laddered (D-P22-1)

- Date: 2026-08-05
- Status: accepted (P22.S1, operator-directed)
- Context: `web/src/app/(app)/graph/graph-canvas.tsx` is a faithful port of the mkdocs
  `docs/javascripts/graph.js`, quiet-labels **A′** included: doc titles fade in past ~110% of fit
  zoom (saturated ~135%) and any focus reveals a whole **neighborhood** of titles. On the app's
  real per-tenant corpus this reads as a wall of titles; the operator asked to "see only clicked
  one". The DOM hover tooltip cannot absorb the loss — `updateTooltip()` bails unless
  `displayZoom() < 0.6`, so it is a low-zoom fallback, not a hover affordance.
- Decision: Rewrite `labelTarget()` as a **selection-driven** rule — `0` outside the active keep
  map, `1` for the **`selectedId`** and for the single **`currentFocus()`** node (drag > hover >
  selection), `0` otherwise — and **delete `ladder()`**. Zoom stops influencing labels entirely.
  Keep the hovered term (rather than the operator's literal "clicked only") because without a
  tooltip above 0.6× zoom, hover would otherwise identify nothing; it is a one-token drop if the
  operator prefers the literal reading. Change **label alpha only**: node dimming, `computeKeep()`,
  halo, ring, edge activation, `drawLabel()` and the tooltip are untouched.
- Scope: **app surface only.** `docs/javascripts/graph.js` keeps the ladder, so A′ is now **narrowed
  to the public mkdocs map**, not superseded. This is the first deliberate fork in the port, and
  there is **no automated parity gate** between the two files — the divergence is held by an
  explicit "DELIBERATE DEVIATION" paragraph in the component's header comment.
- Alternatives considered: (a) strictly selected-only — rejected for the tooltip gap above, and
  reversible in one token; (b) keeping the ladder but raising its threshold — rejected, it only
  moves the wall; (c) extending the DOM tooltip to all zoom levels — rejected as a new interaction
  affordance outside the phase's scope.
- Consequences: the app map is titleless at rest (at most one title with a selection, two while
  hovering a selected node's neighbor); tag/ghost nodes now label on selection/hover like docs;
  both the member `/graph` and the public `/graph/{org}` inherit it from the one shared component;
  a future port sync must not "repair" the missing ladder. An operator browser eyeball is owed (qa).
- Source: P22.S1 `result.md`; P22 `phase.md`; P22.REVIEW.

### Document version history lives on disk (sidecar `.versions/` archive), the DB tables are derived

- Date: 2026-08-05
- Status: accepted (P23 — pinned at `P23.DECOMP`, implemented at P23.S1)
- Context: P23 had to keep superseded document bodies readable. The obvious shape — a
  `document_versions` table holding the old bodies — is **wrong on this plane**: `server/reindex.py`
  rebuilds every `documents` row from the canonical files, and `server/db.py::init_db` will
  drop-and-recreate the content tables on a pre-tenancy DB, so a DB-only history would be silently
  wiped by the next boot reindex. Git is not a substitute either — non-#1 tenants' `tenants/<uuid>/`
  trees are gitignored and never committed, so only tenant #1 would have any history at all.
- Decision: **archive superseded bodies as files, in a reserved top-level `.versions/` dir inside the
  same content root** — `<root>/.versions/<project>/<date>-<slug>/vNNNN.<ext>` — each carrying the
  same frontmatter flavor as the document it came from (so it is self-describing), while the
  **current** version stays exactly where it is at its unchanged `rel_path`. `documents.version` and
  the `document_versions` table are **derived projections** rebuilt from that tree by reindex, exactly
  like `documents` itself.
- Why **top-level** and why **dot-prefixed**: `reindex._walk_root`, `scripts/graph_hook.py` and
  `scripts/site_smoke.py` already reason about **reserved top-level dirs**, so the exclusion is a
  one-constant change in each rather than a filter inside every glob; and mkdocs' default file-scanner
  exclusion is the gitignore spec `['.*', '/templates/']`, so a dot-prefixed dir is skipped at any
  depth and `docs/.versions/` never reaches the built site with no `exclude_docs` entry (verified
  upstream). `docs/versions/` was already taken by the agentic durable docs.
- Alternatives considered: bodies in SQLite only (rejected — erased by the next reindex); git history as
  the archive (rejected — gitignored for every non-#1 tenant); a per-project `<project>/.versions/`
  (rejected — three walkers would each need a nested filter instead of one constant).
- Consequences: **hidden dirs are not skipped by `pathlib`**, so three walkers had to exclude `.versions`
  explicitly — missing the `graph_hook` one would have broken the deploy gate, since archived bodies
  carry a `source:` mapping and would have become graph nodes, breaking `site_smoke.check_graph`'s
  doc-count identity. `server/seed.py` imports `reindex.RESERVED_DIRS`, so it inherited the exclusion for
  free (otherwise `.versions` would have been provisioned as a *project*). Version history is therefore
  as durable as the documents beside it — committed to git for tenant #1, on-box-only for the rest — and
  a full DB wipe + reindex reproduces it. **No Alembic migration**: the content plane has none (the four
  migrations are the Postgres accounts plane only), so this is idempotent DDL in `server/db.py`.
- Source: P23 `phase.md` (pinned design + findings F1/F3); `P23.S1` `result.md`; P23.REVIEW.

### Version rows are identity-keyed, not id-keyed; only the current version is searchable

- Date: 2026-08-05
- Status: accepted (P23.S1)
- Context: `document_versions` is disposable, and a rebuild reassigns `documents.id`. Meanwhile the
  archived bodies are real explainer text that FTS, embeddings and the graph would happily index.
- Decision: key `document_versions` on **`UNIQUE(tenant_id, rel_path, version)`** — the same identity
  `documents` is unique on — with **no FK to `documents`**, so the index survives both an id-reassigning
  rebuild and the pre-tenancy `DROP TABLE documents` path. And index **only the current version**:
  archived rows never enter `documents_fts`, are never embedded, and never become graph nodes.
- Consequences: a route keyed on a document **id** must resolve it to a `rel_path` first (both planes
  do); an archived version has **no stable id** and none is ever exposed in JSON. Search, embeddings,
  the graph and the `/documents` listings are byte-identical to pre-P23 for an unversioned corpus —
  which is what makes the whole feature invisible until someone uses it.
- Source: `P23.S1`/`P23.S2` `result.md`; P23 `phase.md`.

### Identity is immutable across a version chain (a moved project or format is a 422, never a coercion)

- Date: 2026-08-05
- Status: accepted (P23.S1)
- Context: a versioned re-publish keeps the target's `rel_path`, which encodes both the project dir and
  the file extension — and the archive dir sits beside it. A request whose `project` or `format` differs
  from the resolved target's is therefore asking for something the path cannot express.
- Decision: **422, never a silent coercion** ("publish it as a new document instead"). Symmetrically, a
  resolved target's `date` and `slug` **win over the request's** — a v2 sent on a later date keeps the
  original path, and the response echoes the target's values. A target that resolves to nothing is **not**
  an error: it falls through to an ordinary v1 create, so an agent needs no 404 dance.
- Consequences: a document's id, URL, graph edges and inbound `related` links are stable across its whole
  version chain — that stability *is* the feature. The `(project, slug)` convenience path cannot reach
  either 422 guard (only an explicit `rel_path` can). One caveat the skill has to know: `slug` defaults to
  `slugify(title)`, so a **retitled** v2 must send `slug` or `rel_path` or it creates a new document.
- Source: `P23.S1` `result.md`; P23 `phase.md`.

### `overwrite: true` becomes non-destructive and shares the version chain

- Date: 2026-08-05
- Status: accepted (P23.S1) — **the phase's one behavior change to an existing contract**
- Context: `overwrite: true` replaced the file and the DB row outright; prior content was recoverable only
  from git, and only for tenant #1. Once the write path could archive, leaving the legacy path destructive
  would have been a deliberate data-loss branch.
- Decision: `overwrite` now **archives the body it replaces** and bumps the version too (archive v`N`,
  publish v`N+1`), so both write modes share one chain. Status and every pre-existing response key are
  unchanged; `version` / `previous_version` / `archived_path` are additive. `new_version` is documented as
  the primary retry after a 409 and `overwrite` is demoted to "replace rather than supersede".
- Alternatives considered: leaving `overwrite` destructive (rejected — it is the one path that loses data,
  and no consumer wants that); removing `overwrite` (rejected — the CLI and the frozen consumer contract
  expose it, and removing it would be a breaking change).
- Consequences: the CLI (`--overwrite` + its 409 explainer) keeps working untouched because the response
  shape and the 409 detail's four existing keys are preserved; a `--new-version` CLI flag is explicitly out
  of scope. Deleting a document deletes its whole archive — the counterpart decision, so a "deleted"
  document leaves no readable body on disk or in the public git tree.
- Source: `P23.S1` `result.md`; P23 `phase.md`; P23.REVIEW.

### `raw_html` may leave `/api` — but only on an archived version fetch

- Date: 2026-08-05
- Status: accepted (P23.S2) — a **bounded, documented exception** to a pinned rule
- Context: the document surface pins "`raw_html` never leaves `/api` JSON" (a current HTML explainer's raw
  bytes reach a browser only through the sandboxed `/app` raw route). But `.versions/` is never served by
  anything, so a superseded explainer body would be unreadable by an agent through any route.
- Decision: `GET /api/documents/by-path/{rel}/versions/{version}` returns `raw_html` **when the version's
  format is html** — and only there. The `/app` plane keeps the rule intact (its version JSON never carries
  `raw_html`; the archived bytes leave through `.../versions/{v}/raw` under the same four sandbox headers as
  the current-document raw route).
- Consequences: the exception is one route on the machine plane, where the caller is an agent that already
  holds a tenant-scoped credential; the browser plane's containment contract is unchanged, since a
  superseded explainer is exactly as untrusted as a live one. Any future widening of this exception should
  be a new ADR, not an extension of this one.
- Source: `P23.S2` `result.md`; P23 `phase.md`; P23.REVIEW.

### The public URL namespace is `/@{org}` — documents at `/@{org}/{project}/{doc-slug}`, the graph at `/@{org}/graph`

- Date: 2026-08-07
- Status: accepted (P25 decomposition; verified empirically in P25.S3/S4)
- Context: a shared document was `/documents/25` (a disposable SQLite rowid) and the public graph
  `/graph/{tenant-uuid}`. Making public URLs human-readable means putting an org name at or near the root
  of the path, which risks colliding with every existing and future top-level route (`/documents`,
  `/graph`, `/dashboard`, `/login`, `/projects`, `/api/*`).
- Decision: prefix the whole public namespace with `@`. The `@` marks the namespace explicitly, so new
  top-level app routes can be added forever without collision risk and no reserved-name list is needed for
  **routing** (a reserved list is still needed for slug *provisioning*, for a different reason — see below).
  The graph lives **under** the org namespace (`/@{org}/graph`, two segments) rather than `/graph/@{org}`,
  keeping one rule: everything public hangs off `/@{org}`.
- Alternatives considered: a bare `/{org}/…` path (rejected — a root-level dynamic segment with no marker
  makes every future top-level route a potential collision); a `/docs/{org}/…` or `/s/{org}/…` prefix
  (rejected — an extra opaque segment that says nothing to a reader); `/graph/@{org}` (rejected — two
  namespaces to remember instead of one).
- The Next.js parallel-route hazard does **not** apply: a folder literally named `@x` is a slot, but the
  route folder here is a plain dynamic `[org]` whose *param value* happens to start with `@`, which is a
  legal unencoded path character.
- Consequences, verified rather than assumed: a root-level `[org]/[project]/[slug]` **shadows nothing** —
  Next compiles full-path regexes and orders static routes before dynamic ones, sorting this all-dynamic
  route last (proven against the built `routes-manifest.json` and by a live `next start` probe), so the
  planned fallback (a `next.config` rewrite or a `middleware.ts` onto an internal prefix, with the same
  user-facing URLs) was **not needed**. The flip side: the pretty document route is now the catch-all for
  every unmatched 3-segment URL, so any later 3-segment route must carry a static prefix to win — which is
  what makes the `@`-prefix guard (rejecting a non-`@` org segment before any session read or upstream
  call) load-bearing rather than cosmetic.
- Source: P25 `phase.md` (D1); `P25.S3` / `P25.S4` `result.md`; P25.REVIEW.

### The pretty document address omits the date and resolves to the newest — and only the row it resolves to may advertise it

- Date: 2026-08-07 (guard added by `P25.F1` after the first review pass)
- Status: accepted (P25 decomposition, amended by P25.F1)
- Context: a document's durable identity is `rel_path = "<project>/<date>-<slug>.<ext>"`, so a faithful
  pretty URL would carry the date. But a share link reads better — and means more — without it.
- Decision: `/@{org}/{project}/{doc-slug}` is **dateless** and resolves via `find_latest_document_by_slug`
  (`date DESC, id DESC`, tenant-scoped): *"the newest document under this title"*. No dated form is
  introduced. Exact-row addressing already exists and keeps working (`/documents/{id}`, and
  `/documents/{id}/versions/{v}` for a specific body).
- Rationale: "the latest doc under this title" is what a shared link should mean, and it matches the P23
  versioning story — a re-publish under the same slug *is* the new version.
- **The amendment (P25.F1), and why it was needed.** As first built, the dateless path was composed for
  *every* document and the anonymous `/documents/{id}` branch redirected onto it. Duplicate `(project, slug)`
  rows at different dates are reachable in ordinary use (a publish **without** `new_version` computes a
  different `rel_path`, so the collision check finds nothing and inserts a second row), and the composition
  therefore pointed an **exact-row** address at a **newest-wins** one: an already-shared `/documents/{id}`
  link 307'd to a *different* document, and copy-link handed out the same wrong URL. Confirmed empirically
  at the P25 review, not inferred. The fix: emit `canonical_path` (and the pretty 201 `url`) **only when the
  path round-trips back to that same row**. The invariant is now *a non-null `canonical_path` always resolves
  back to the document that advertised it*; a superseded duplicate answers `null` and keeps its id link.
- Alternatives considered: keeping the composition and documenting the behavior (rejected — a share link
  that quietly changes what it shows is worse than one that stays on an id URL); introducing a dated pretty
  form (rejected — it re-introduces the unreadable component the phase exists to remove, and the id URL
  already addresses an exact row).
- Consequences: `null` needed **zero** web changes — every consumer already handled it, because that is the
  slug-less contract; superseded duplicates simply join it. Re-publishing the same slug on a later date
  still moves the pretty URL's target, which is the intended meaning of the address. The resolver route's
  own `canonical_path` stays unconditional (it resolved *through* the slug, so the check would be a
  tautology). The guard costs one SQLite read on the connection already in hand, evaluated **after** the
  slug check, so a slug-less deployment pays nothing.
- Source: P25 `phase.md` (D2, and § "Found by P25.REVIEW"); `P25.F1` `result.md`; P25.REVIEW.

### Org slugs are nullable, operator-set, and mutable — no backfill, no derived default, no slug history

- Date: 2026-08-07
- Status: accepted (P25 decomposition)
- Context: the org slug is the durable public identity a pretty URL is built from. Somebody has to choose
  it, and the phase's hardest constraint is that nothing which works today may stop working.
- Decision: `tenants.slug` is `text NULL UNIQUE`, set by the operator through `PATCH /app/tenant` and the
  dashboard's Public URL panel. **No backfill, no derived-from-name default, and no hardcoded value for
  tenant #1** — a tenant without a slug keeps exactly today's URLs, and the operator claims one after
  deploy. Charset `^[a-z0-9]+(-[a-z0-9]+)*$`, 2–40 chars, lowercased on write, plus a 38-word reserved-word
  guard (validated app-layer, no DB `CHECK`, matching the `projects.visibility` precedent). Slugs are
  **mutable** and there is **no slug-history / old-slug redirect**.
- Alternatives considered: deriving a slug from the org name at signup (rejected — it would mint public
  identities nobody asked for, and a bad auto-name is worse than none); making the column `NOT NULL` with a
  backfill (rejected — it forces a public name on every existing tenant and turns a pure additive migration
  into a data migration); immutable slugs (rejected — with one operator and no slug claimed yet, the cost of
  a mistake is zero and the flexibility is real).
- Consequences: a slug-less tenant sees byte-identical pre-P25 behavior everywhere, which is what makes the
  migration a zero-risk pure add — and it is also the fallback path every consumer already exercises.
  Changing a claimed slug **breaks links already shared under the old one**; that cost is accepted for now
  and is named inline in the dashboard hint (`P25.F2`) so it is not a silent trap. If the operator ever
  disagrees, the fix is a `tenant_slug_history` table in a later phase, not a change to this shape.
  Uniqueness here is **global** — unlike every other constraint in that schema, which is tenant-scoped —
  which is a standing hazard for tests (generate the slug per run) as much as a product property.
- Source: P25 `phase.md` (D3, Q3); `P25.S1` / `P25.S5` / `P25.F2` `result.md`; P25.REVIEW.

### Old links redirect on the anonymous surface and dual-serve for members — temporarily, never permanently

- Date: 2026-08-07
- Status: accepted (P25 decomposition)
- Context: `/documents/{id}` and `/graph/{uuid}` are already in the wild. The phase's constraint is that
  redirecting is allowed but 404ing is not — and the id URL carries member-only affordances (back-link,
  delete) the pretty page deliberately does not.
- Decision: an **anonymous** visitor on `/documents/{id}` is redirected to `canonical_path` when there is
  one; a signed-in **member** stays on the id URL. `/graph/{uuid}` redirects on **both** branches, because
  that surface has no member-only affordance to preserve. Every redirect is **307 (temporary)**, never 308,
  precisely because slugs are mutable — a permanent redirect would be cached by browsers past a slug change.
  Every redirect path has a "serve exactly as today" fallback when there is no slug.
- Alternatives considered: 308 permanent redirects (rejected — incompatible with mutable slugs); redirecting
  members too on the document page (rejected — it strips affordances, and intent explicitly allows internal
  navigation to keep ids); rewriting doc-node `url`s in the graph payload to the pretty form (rejected as
  out of scope — they resolve through the same redirect).
- Consequences: redirects are **one-way** — the pretty pages never bounce back to an id URL, so no loop is
  possible — and a pretty-URL miss is a branded not-found on both branches rather than a `/login` bounce,
  since the resolver's single 404 already collapses private/unknown and a bounce would only add friction on
  a share surface. Because the anonymous document redirect targets a *dateless* address, it is exactly the
  composition the round-trip guard above had to make safe.
- Source: P25 `phase.md` (D4); `P25.S3` / `P25.S4` `result.md`; P25.REVIEW.

### Two smaller P25 resolutions, recorded so they are not re-litigated

- Date: 2026-08-07
- Status: accepted (P25, Q2 + Q4)
- **The project segment is matched exactly (case-sensitive).** Project names may contain uppercase
  (`^[A-Za-z0-9][A-Za-z0-9._-]*$`) and the name is a directory name on disk, so an exact match is the
  simplest unambiguous rule. A case-insensitive lookup that canonical-redirects is a nicety, deliberately
  deferred rather than built.
- **The 201 save `url` is pretty even for a *private* project.** The resolver is optional-identity, so the
  pretty URL resolves for the author regardless of visibility, and consulting visibility on the write path
  would buy a lookup for nothing. The URL is shareable only once the project is public — exactly like
  `/documents/{id}` has been since P19.
- Source: P25 `phase.md` (Q2, Q4); `P25.S2` `result.md`; P25.REVIEW.
...
- The **P6.S0 "Label Strategy A" lock (doc titles always on)** is **superseded** by the operator-directed P6.S1 quiet labels (Strategy A′): the idle map is marks-only; titles reveal on hover/selection and above ~110% zoom. **(P22)** A′ as stated here now describes the **public mkdocs map only** — the app's ported `GraphCanvas` dropped the zoom ladder and the neighborhood reveal for selection-driven labels (D-P22-1); A′ is **narrowed, not superseded**.
...
- **D-P18's "the `init` reuse-gate keeps its structure verbatim, so `init --project other` re-mints"** consequence — flagged there as a candidate deferred follow-up (D16) — is **resolved** by D-P20-2: the reuse-gate now reuses the org key across projects (org keys are project-agnostic), so `init --project other` no longer re-mints. The rest of D-P18 (the `"default"` retarget, org-key mint on `init`, same-service gate, `--new-key` re-mint) is **unchanged**.
