...
doc_id: experience
version: v0017
created_at: 2026-08-07T17:50:26+09:00
source: P25.REVIEW
summary: P25.F3 the silent id-URL fallback gets a voice: an unclaimed-org-name hint beside the member copy affordances, linking to the dashboard Public URL panel
previous: v0016_p25_the_shared_link_becomes_readable_and_durable_claim_an_org_slug_in_the_dashboard_org_pretty_pages_old_links_307-forward_and_a_superseded_duplicate_keeps_its_id_link
---
...
- **The fallback stopped being silent (P25.F3).** Shipping the phase to production exposed the gap the design did not: with no name claimed yet, every share affordance handed out the old id/UUID link *correctly and wordlessly*, which reads as "the pretty-URL feature is broken". Now, on the **member** branch of the document read view and in the **member graph header**, a viewer whose **own org** has claimed no name sees one short sentence beside the copy button — *"This link uses an id until you [claim a public URL name]."* — linking straight to the dashboard's Public URL panel. Three rules keep it quiet: it is shown **only to the person who can fix it** (it keys off the viewer's own org name, never off the document's), **never once a name is claimed** (a pretty-path-less document under a claimed name is the P25.F1 superseded case, whose id link is correct and permanent — there is nothing to do), and **never on an anonymous branch** (a visitor can claim nothing and already has the URL in the address bar). The pretty document page carries no hint either: reaching it means a name resolved.
- **Known residual edge (P25.F3).** The document read view's member branch is *not* own-org only — a member of one org can reach another org's public document there — and the web plane carries no ownership signal, so the hint's second condition ("this document advertises no pretty path") is a proxy. It excludes the misleading case (another org's document that *does* have a pretty URL) but still shows the prompt on another org's *name-less* document, where the sentence is true of the viewer's own links yet cannot change that particular one. Closing it needs a new backend ownership field, not a TypeScript inference.
