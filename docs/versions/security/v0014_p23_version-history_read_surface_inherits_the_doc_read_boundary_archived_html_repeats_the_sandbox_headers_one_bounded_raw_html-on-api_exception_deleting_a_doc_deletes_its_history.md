...
doc_id: security
version: v0014
created_at: 2026-08-05T23:01:51+09:00
source: P23.REVIEW
summary: P23 version-history read surface inherits the doc read boundary; archived HTML repeats the sandbox headers; one bounded raw_html-on-api exception; deleting a doc deletes its history
previous: v0013_p21_member-only_web_delete_boundary_session-only_delete_with_no_public_fallback_404-never-403_reading_a_public_doc_never_implies_deleting_it
---
...
**P16 introduces the first document format whose content is *executable untrusted code* — a self-contained HTML explainer with inline JS — and contains it by construction rather than sanitizing it away.** The web app's markdown viewer was "XSS-safe by construction" (react-markdown, no `rehype-raw`); an HTML explainer must instead *run* its quiz JS. P16 keeps the safety stance while allowing that interactivity via an **opaque-origin sandboxed iframe**: `sandbox="allow-scripts"` **without** `allow-same-origin`, pointed at a same-origin BFF raw-relay route, with the raw response also carrying a `Content-Security-Policy: sandbox allow-scripts` (defense in depth) and an `X-Frame-Options` exemption scoped to that one path. The untrusted explainer JS runs but has **no session cookie, no storage, no cross-origin parent DOM, and no way to call the API as the user**; the raw HTML never appears in a JSON body, and search indexes only extracted text. See *HTML explainer XSS containment (P16)* below.

**P18 makes org-level API keys honest, and it is a wording change to this doc — not a trust-boundary shift.** A `vk_` is now mintable at **org level** (one key grants the whole org/tenant). Crucially, enforcement was **already** tenant-wide before P18: the resolver returned `ctx.tenant_id` for any `vk_`, and the write path never checked a document's `project` against the credential — the key's project binding only ever drove usage attribution. So org-level keys make the de-facto behavior explicit; they do **not** loosen isolation. Every P10 boundary holds unchanged (per-tenant content isolation, cross-tenant → 404, sha256-at-rest tokens, argon2id, enumeration-safe 401s), and a **revoked** org key resolves to nothing → **401**, exactly like a project-bound one. See *Org-level key authorization made honest (P18)* below.

**P19 opens the product's first *anonymous* read path — the biggest read-surface trust-boundary widening since P10 — and does it fail-closed.** A project can be toggled public; a public project's docs and its knowledge graph become readable with **no session at all**. The boundary is built so private and nonexistent are indistinguishable everywhere on the new surface (**404-never-403**, no 403 anywhere), and it defaults closed at every layer: a new **optional-identity** dependency (`optional_user`) that **never raises** (a miss yields the anonymous 404, not a 401); a per-read Postgres→SQLite **visibility bridge** whose SQLite predicate **fails closed on an empty allowlist**; a **legacy-mode guard** so the single-tenant template stack never exposes a public path; registry-less rows that are **never public**; a graph public path that **404s a nonexistent/no-public org** (no existence leak); and a web raw-HTML relay that goes **tokenless** for an anonymous visitor (never fetch-with-token on their behalf). No new secret, credential store, CORS change, or PII. The one recorded gap: the anonymous read surface has **no per-IP rate limit** yet (only `/auth/*` is throttled) — a deferred follow-up. See *Anonymous public read surface (P19)* below.
...
- **(P21) The `/app` web plane: session-only, and destructive where it writes.** Every `/app/*` route is session-guarded except the P19 optional-identity public reads. The one mutating route, `DELETE /app/documents/{doc_id}`, is **`require_user` only — it never inherits the public read fallback** (see *Member-only delete on the web plane* below). Any future `/app` write must make the same choice explicitly.
...
## HTML explainer XSS containment (P16)

P16 makes a standalone interactive HTML explainer a first-class document type. Its content is **untrusted executable code** (inline JS), so the security question is *how does the quiz run without becoming an XSS vector*. The answer is containment, not sanitization (sanitizing would kill the quiz — the whole point).

- **Opaque origin via `sandbox="allow-scripts"` without `allow-same-origin`.** The web viewer frames a `format === "html"` document in `<iframe sandbox="allow-scripts">` — and crucially **never** `allow-same-origin` (nor allow-forms/popups/top-navigation/modals). Omitting `allow-same-origin` gives the framed document an **opaque origin**: no `document.cookie`, no `localStorage`/`sessionStorage`/IndexedDB, and `window.parent`/`window.top` DOM access is cross-origin (blocked by the same-origin policy). `allow-scripts` alone keeps the multiple-choice quiz working.
- **Cannot exfiltrate the session or act as the user.** The app session cookie is `httpOnly` (JS can't read it regardless), **and** the `/app` + `/api` planes have **no CORS** (server-to-server) — so a credentialed cross-origin `fetch` from the opaque iframe to the API is blocked. The untrusted explainer JS therefore can neither read the session nor call the API as the signed-in user.
- **Defense in depth for a direct top-level visit.** The raw bytes are served with `Content-Security-Policy: sandbox allow-scripts; frame-ancestors 'self'`, so a person who navigates **straight to the raw URL** gets the sandbox applied to the top-level document too — also privilege-stripped. `frame-ancestors 'self'` means only the app may frame it. The relay sets `referrerPolicy="no-referrer"` on the iframe so the relay URL doesn't leak in a referer.
- **The X-Frame-Options exemption is scoped and consistent.** The global `X-Frame-Options: DENY` (which blocks even same-origin framing) is exempted **only** for `/api/documents/:id/raw` → `SAMEORIGIN`; the parent document page keeps `DENY`. The CSP + X-Frame pair is set **identically** at the backend raw route, the BFF relay, and the `next.config` header entry, so no header-layer order can resolve to a permissive value (verified at runtime).
- **Raw HTML never crosses a JSON boundary; search sees only extracted text.** The `raw_html` column is dropped from every JSON projection (`/api/*` and `/app/*` reads) — only the dedicated `text/html` raw route (session-guarded, tenant-scoped, 404 on missing/cross-tenant/non-html) serves it. Full-text search / snippets / embeddings index the **extracted** text, so an explainer is never findable by, and never surfaces, its `<script>`/`<style>` contents (verified at the P16 review E2E).
- **Auth + tenancy unchanged.** The raw route is `require_user`-guarded and tenant-scoped exactly like the rest of `/app/*` (cross-tenant → 404, existence never leaks); the web relay self-guards on the sealed session cookie (401 with no upstream call when absent). No new credential, secret, CORS change, or auth surface — P16 widens the *content* threat model (executable doc content) but not the auth model.
- **Residual (operator visual acceptance).** The live in-browser confirmation that the quiz runs while a direct raw-URL visit is sandbox-stripped needs a real browser (no jsdom/browser tooling in the repo); it is an operator residual. Every machine-verifiable layer — the sandbox attribute, the identical CSP/X-Frame across all three layers, the runtime header check, extracted-text-only search, and `raw_html` never in a JSON body — passed at the P16 review.

## Org-level key authorization made honest (P18)

P18 makes `vk_` keys mintable at **org level** (one key grants the whole org/tenant). This is a **wording change** to the auth model, not a boundary change — recorded so no one mistakes it for a loosening:

- **Enforcement was already tenant-wide.** Since P10 the `/api/*` resolver mapped any `vk_` → `ctx.tenant_id`, and the write path authorized by tenant and **never** checked a document's `body.project` against the credential (`validate_project` is a convention check only). The key's project binding drove **usage attribution**, not authorization. Org-level keys make that explicit — they authorize exactly what a project-bound key already did (the whole tenant).
- **The resolver reads the credential's own `tenant_id`.** An org key carries `project_id NULL` and resolves to its `tenant_id` with no bound project; a project-bound key keeps its `project_id` and the vanished-project existence guard. No cross-tenant reach is introduced (a credential still only ever resolves to its own tenant).
- **Revocation is unchanged.** A revoked org key resolves to nothing → **401**, identical to a revoked project-bound key (verified in the Postgres-gated `test_org_credentials` and live at the P18 cutover — revoke → subsequent write 401).
- **No new secret, credential store, CORS change, or PII.** Org keys are stored exactly like project-bound ones (sha256-hex `token_hash` + `token_prefix`, raw key shown once at mint, never persisted). The `0003` schema adds a backfilled `tenant_id` column and makes `project_id` nullable — no credential material or trust boundary changes.

## Anonymous public read surface (P19)

P19 makes a project's docs + graph readable without a session when the project is public. It is the first anonymous read path on prod, engineered to fail closed at every layer:

- **Optional-identity that never raises.** `optional_user` (`server/accounts/auth.py`) resolves a bearer exactly as `require_user` but **returns `None`** on every miss (no header, unknown/expired token, missing user, no tenant) rather than raising. So an unresolvable token on a private doc yields the anonymous **404**, not a 401 — it leaks strictly less (no "this id exists" signal). Only `GET /app/documents/{id}` + `/raw` + `GET /app/graph` use it; `/app/documents` (list) + `/app/search` stay `require_user` (session-only).
- **404-never-403, private == nonexistent.** Doc read (`_resolve_readable_doc`): a member fast-path serves the caller's own tenant byte-preserved; otherwise the doc is served **only** if its project is `visibility == "public"` — a missing id, a private doc, a cross-org doc, and a nonexistent id are **all indistinguishable 404s**. Graph: a non-member `org` with no public projects (or a nonexistent org) → **404 `graph not found`** (no existence leak). There is **no 403** anywhere on the surface.
- **The visibility bridge fails closed.** Project identity/visibility lives in Postgres; the SQLite `documents` rows carry only the project **name string**. Each read resolves the owner's public-project name set from Postgres and passes it as a `db._filtered(projects=[…])` allowlist (`project IN (?,…)`). The default `None` leaves every legacy caller byte-identical; an **empty** allowlist is `WHERE … 0` — matches nothing — so the predicate can never open up on an empty set (defense in depth behind the graph route's own empty-set 404).
- **Registry-less rows are never public; legacy mode never exposes a public path.** A doc whose `tenant_id` is empty / the `''` legacy sentinel / an unparseable UUID resolves to 404 (never public). Both the doc and graph public paths short-circuit to 404 when `config.database_url()` is `None` **before** touching the (absent) accounts service, so the single-tenant/template stack never 500s and never serves a public read.
- **The web boundary is anonymous-tokenless.** The public doc page (`(public)/documents/[id]`) fetches **tokenless** on the anonymous branch (`getDocument(undefined, id)`) and never renders a token-fetched doc to an anonymous visitor; a private/nonexistent miss uniformly **redirects to `/login`**. The optional-identity **raw-HTML relay** (`/api/documents/{id}/raw`) forwards the bearer only when a cookie is present, else relays **tokenless** (`openSession(...) ?? undefined`) — an anonymous browser sends no cookie, so it can never fetch-with-token on the visitor's behalf. The four P16 sandbox headers are **byte-identical** at the backend raw route, the BFF relay, and `next.config`, so no header-layer order resolves permissive; `robots.ts` now allows crawling `/documents` + `/graph` (private/nonexistent still 404 or bounce, leaking nothing).
- **No new secret / credential / CORS / PII.** Visibility is a boolean-ish `text` column added by `0004` (`projects.visibility`, default `'private'`); no token, credential store, CORS policy, or personal data changes. The toggle (`PATCH /app/projects/{id}`) is session-only and cross-tenant-404.
- **Recorded gap — no anonymous-read throttle.** The new public doc/`raw`/graph paths have **no per-IP rate limit** (only `/auth/*` is throttled). A future edge or in-process limiter on the anonymous read paths is a deferred follow-up; it does not block the phase.

## Member-only delete on the web plane (P21)

P21 gives the `/app` plane its first **destructive** capability — a session-guarded hard delete of a document — so the surface that P19 opened to anonymous readers now also carries a write. The two are deliberately not symmetric:

- **The read fallback is not inherited by the delete.** `GET /app/documents/{id}` + `/raw` are optional-identity: an anonymous or cross-org caller may read a *public* project's doc. `DELETE /app/documents/{doc_id}` is **`require_user` with a tenant-scoped lookup only** — no `optional_user`, no `_resolve_readable_doc`, no public path. **Reading a document never implies deleting it**, and this asymmetry is the rule to preserve whenever a route is added to this prefix.
- **404-never-403, as everywhere else on this surface.** Anonymous → generic **401**; a nonexistent id and another tenant's id are **indistinguishable 404s** (`no document with id <id>`), so the route is not an id oracle and a cross-tenant delete is impossible by construction rather than by a check that could be forgotten. There is no 403. Verified live at the P21 review: a member's delete of another org's document returns the not-found copy and the target survives.
- **Authorization is the query, not a branch.** The worker's only lookup is `db.get_document(conn, doc_id, tenant_id=<caller's tenant>)`; a miss is the 404. There is no "is this mine?" comparison to get wrong, and a forged/tampered id in the browser form buys nothing — the web layer's own zod validation is a cheap rejection, not the boundary.
- **CSRF: a server action, not a route handler.** The web delete is a Next.js `"use server"` action behind `requireIdentity()`, so it inherits the framework's server-action CSRF protection and the sealed-cookie BFF posture (P12) unchanged — the knowledge bearer is unsealed server-side and never reaches the browser. No new route handler, no new CORS, no new credential or secret.
- **Open-redirect closed by construction.** The action's post-delete destination arrives as a hidden form input and is validated with a **`z.literal("/documents")`** — a literal, never a free-form string — so a tampered `redirectTo` cannot become an open redirect; a failed parse simply means "stay put".
- **New destructive authority, unchanged blast radius.** The delete reuses the machine plane's existing `_delete_document`, so it can destroy exactly what a `vk_` key already could: it is a **hard, irreversible** delete (file, DB row, FTS entry, embeddings) with **no trash and no undo**, non-transactional (the file goes first), and for a tenant-#1 member it also updates the public Recent index and produces a real git commit. Deleting a public document breaks its shared URL and its graph edges — both derived and self-healing, and the confirm copy promises no recoverability.
- **Unmetered on purpose, and that is a boundary choice too.** The route sets no `request.state.usage`. The rejected alternative — calling the metered `DELETE /api/documents/{id}` with the user's session bearer (`resolve_api_write` accepts one) — would have routed the web app through the machine plane's auth resolver, blurring the two planes' trust boundaries, in addition to billing a UI click. Reusing the *logic* rather than the *route* keeps the planes separate.
- **Recorded, accepted gap.** A member reading **another org's public** document sees the delete control and gets the 404 copy on submit: there is no client-side tenant signal to hide the button by, and 404-never-403 means the server must not tell them why. This is a cosmetic gap, not an authorization one — the delete is refused server-side.

## Version history: a wider read surface on the same boundary (P23)

P23 makes superseded document bodies readable through five new routes. The threat-model shift is **narrow by construction** — every route reuses an existing boundary rather than adding one — but three things are worth pinning.

- **The archive inherits the document's read boundary exactly.** Each `/app` version route resolves the **document** first through `_resolve_readable_doc` (member fast-path → public-project fallback), and only then reads the archive by that document's `rel_path`. So a version is readable precisely when its current document is: private stays private, and a missing document, a private one, a missing version and the *current* version number are all the same indistinguishable **404** — never a 403, and never an id oracle. The `/api` routes gate the same way on the tenant-filtered current document.
- **The archive is scoped to the OWNING document's tenant, not the caller's.** An anonymous reader of a public document has no tenant of their own, so the version lookup carries the document row's `tenant_id`. That is what lets a public document's history be read anonymously **without any cross-tenant read ever becoming expressible** — the scope is derived from the row that already passed the read gate, not from user input.
- **An archived HTML explainer is exactly as untrusted as a live one.** `GET /app/documents/{id}/versions/{v}/raw` repeats the P16 sandbox headers **verbatim** (`Content-Security-Policy: sandbox allow-scripts; frame-ancestors 'self'`, `X-Frame-Options: SAMEORIGIN`, `X-Content-Type-Options: nosniff`, `Cache-Control: no-store`), and the web BFF relay re-asserts them explicitly rather than copying them from upstream — plus a `next.config.ts` per-path entry, without which the global `X-Frame-Options: DENY` would win. The opaque-origin containment of *HTML explainer XSS containment (P16)* applies unchanged; nothing was softened for history.
- **One bounded exception to a pinned egress rule.** `GET /api/documents/by-path/{rel}/versions/{version}` returns `raw_html` for an html version — the single place raw HTML leaves the `/api` plane, because `.versions/` is served by nothing and a superseded explainer would otherwise be unreadable by an agent. The caller there already holds a tenant-scoped credential, and the `/app` plane keeps the rule intact (its version JSON never carries `raw_html`). Any future widening should be its own decision, not an extension of this one.
- **Reads only, and unmetered.** None of the five routes writes anything or sets `request.state.usage`; no new usage event type exists. Browsing history moves no counter and is free like the rest of the web UI.
- **Deleting a document deletes its history — deliberately.** `_delete_document` removes the whole `.versions/<project>/<date>-<slug>/` dir and its rows, staged in the same scoped commit, so a "deleted" document leaves **no readable body behind on disk or in the public git tree** and reindex cannot resurrect it. This is a data-retention property, not an optimization: do not "soften" the delete to keep history.
- **Publish safety is unchanged.** Archived bodies live under a dot-prefixed reserved dir that mkdocs never builds and the graph never walks, so nothing new reaches the published site. For tenant #1 the archive *is* committed to the public repo — which is correct, since that corpus is already public — while non-#1 tenants' archives stay on-box and gitignored exactly like their documents.
...
- [x] **(P16)** An HTML explainer's untrusted JS runs only in an **opaque-origin** iframe (`sandbox="allow-scripts"`, **no** `allow-same-origin`) — no cookie/storage/parent-DOM/credentialed-API access; the app is CORS-less + the session cookie is httpOnly
- [x] **(P16)** Defense in depth: the raw response carries `CSP: sandbox allow-scripts; frame-ancestors 'self'` (a direct top-level visit is privilege-stripped) and the CSP/X-Frame pair is identical across the backend route, the BFF relay, and `next.config` (verified at runtime)
- [x] **(P16)** `raw_html` is never in any JSON body (dropped from every read projection); the raw route is `require_user`-guarded + tenant-scoped (404 on missing/cross-tenant/non-html); search indexes only extracted text (never `<script>`/`<style>`)
- [x] **(P18)** Org-level `vk_` keys are a **wording change, not a boundary shift** — enforcement was already tenant-wide; org keys authorize the same whole-tenant scope a project-bound key did, resolve only to their own tenant, and a revoked org key → 401 (verified gated + live)
- [x] **(P18)** No new secret/credential store/CORS change/PII — org keys are stored like project-bound (`token_hash` + `token_prefix`, raw shown once); `0003` adds a backfilled `tenant_id` + nullable `project_id`, no trust-boundary change
- [x] **(P19)** The first anonymous read surface is **404-never-403** — private, cross-org, and nonexistent doc/graph reads are indistinguishable 404s; no 403 anywhere (verified gated + live)
- [x] **(P19)** `optional_user` **never raises** — an unresolvable token on a private doc yields the anonymous 404, not a 401 (leaks strictly less); only the three read routes use it, list/search stay session-only
- [x] **(P19)** The Postgres→SQLite visibility bridge **fails closed** — an empty project-name allowlist is `WHERE 0` (matches nothing); registry-less rows (`''`/unparseable owner) are never public; legacy mode (`DATABASE_URL` unset) 404s before touching the accounts service
- [x] **(P19)** The graph public path 404s a no-public/nonexistent org (no existence leak); the doc/raw public path serves only `visibility=="public"` projects
- [x] **(P19)** The web anonymous branch fetches **tokenless** and the raw-HTML relay relays tokenless with no cookie — never fetch-with-token for an anonymous visitor; the four P16 sandbox headers stay byte-identical across backend/BFF/`next.config`
- [x] **(P19)** No new secret/credential store/CORS change/PII — visibility is a `text` column (`0004`, default `'private'`); the toggle is session-only + cross-tenant-404
- [ ] **(P19)** The anonymous read surface has **no per-IP rate limit** yet (only `/auth/*` is throttled) — a deferred follow-up (an edge or in-process limiter on the anonymous doc/`raw`/graph paths)

## Open Questions

- **~~No rate limiting on the hosted vhost, deliberately.~~ Superseded at P13 for the `/auth` grant.** The pre-P13 rationale ("every `/api/*` call is bearer-gated, one known low-volume consumer, so no throttle is needed") **went false the moment P13 published `/auth/{signup,login}`** — an unauthenticated password grant + open signup. P13 added the throttle **app-side** (server-side per-IP, above), which is exactly the "app-side … deliberate change" this note foresaw, chosen over an nginx zone because the edge's global-zone-name ban still holds (a duplicate `limit_req_zone` breaks the whole `conf.d/` tree). The `/api/*` surface stays unthrottled (still bearer-gated, one consumer); only the newly-public `/auth/*` grant is limited. If `/api/*` ever gains untrusted consumers, throttle it the same app-side way. **P19 reopened this for reads:** the new anonymous public doc/`raw`/graph paths are unauthenticated and unthrottled — the pre-P13 "one bearer-gated consumer" rationale does not cover them. They leak nothing (404-never-403, fail-closed bridge), but they are open to volume; a per-IP limiter on the anonymous read paths (edge or the same in-process app-side mechanism, before they see real traffic) is the deferred follow-up. Not blocking the phase — the surface is small and the corpus that is public is public by intent.
